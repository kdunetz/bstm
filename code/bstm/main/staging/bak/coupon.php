...
if (!isset($_GET['coupon_id']) && isset($_POST['coupon_id']))
   $_GET['coupon_id'] = $_POST['coupon_id'];
     $objects_query = tep_db_query("SELECT coupon.*,product.product_name,delivery_mechanism.delivery_mechanism_name,source.source_name, store.store_name FROM coupon LEFT JOIN store on coupon.store_id = store.store_id INNER JOIN source on source.source_id = coupon.source_id INNER JOIN delivery_mechanism on coupon.delivery_mechanism_id = delivery_mechanism.delivery_mechanism_id INNER JOIN product on coupon.product_id = product.product_id WHERE coupon.coupon_id = " . $_GET['coupon_id']);
     ...
     echo "<div id=\"content\">
<a href=index.php>Back to Coupon List</a>";
