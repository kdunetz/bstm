<?php
/**
 * @version $Id: joocmavatar.php 133 2010-07-27 11:45:15Z sterob $
 * @package Joo!CM
...
class JoocmAvatar
{
	...
	function getJoocmAvatarFile($avatar) {
		...
		if ($avatarFile == '') {
			$avatarFile = JURI::root().$this->avatarPath.DL.'standard'.DL.'_cm_noavatar.png';
		}
	...
	function deleteAvatar($avatarId = 0) {
		...
		if ($avatarId == 0) {
			$app->enqueueMessage(JText::_('CM_MSGNOAVATARSELECTED')); return false;
		}
		...
		if ($joocmUser->get('id') <> $avatar->id_user) {
			$app->enqueueMessage(JText::_('CM_MSGNOPERMISSIONMANAGEAVATARS')); return false;
		}
		
		$avatarPath = $joocmConfig->getAvatarSettings('avatar_path');
		if (!unlink(JPath::clean(JPATH_ROOT.DS.$avatarPath.DS.$joocmUser->get('id')).DS.$avatar->avatar_file)) {
			$app->enqueueMessage(JText::_('CM_MSGAVATARFILENOTDELETED')); return false;
		}
	...
	function uploadAvatar($avatarFile, $avatarURL = '', $avatarId = 0) {
		...
		if ($avatarURL == '' && $avatarFile['name'] == '') {
			$app->enqueueMessage(JText::_('CM_MSGNOAVATARTOUPLOAD'), 'notice'); return false;
		}
		
		$userAvatarsCount = count($this->getUserAvatars($joocmUser->get('id')));
		if ($userAvatarsCount >= $this->avatarsMax && $avatarId == 0) {
			$app->enqueueMessage(JText::sprintf('CM_MSGAVATARMAXREACHED', $this->avatarsMax), 'notice'); return false;	
		}
		...
		if ($avatarFile != '') {
			$avatarUserPath = JPath::clean(JPATH_ROOT.DS.$this->avatarPath.DS.$joocmUser->get('id'));
			...
			if (preg_match("/$fileTypes/i", $avatarFile['name'])) {
				...
				if ($joocmConfig->getAvatarSettings('image_resize') && $joocmGD->isEnabled()) {
					...
					if ($avatarFile['size'] > $joocmConfig->getAvatarSettings('avatar_max_file_size')) {
						$app->enqueueMessage(JText::sprintf('CM_MSGMAXIMALFILESIZE', $avatarFile['size'], $joocmConfig->getAvatarSettings('avatar_max_file_size')), 'notice'); return false;
					}

					// check image size
					$imageSize = getimagesize($avatarFile['tmp_name']);
					if ($imageSize[0] > $this->avatarWidth || $imageSize[1] > $this->avatarHeight) {	
						$app->enqueueMessage(JText::sprintf('CM_MSGREQUIREDWIDTHHEIGHT', $this->avatarWidth, $this->avatarHeight), 'notice'); return false;
					}
				}
				...
				if (JFile::upload($avatarFile['tmp_name'], $fileName)) {
					...
					$app->enqueueMessage(JText::sprintf('CM_MSGAVATARFILEUPLOADFAILED', $joocmUser->get('name')), 'error'); $result = false;
				}		
			} else {
				$app->enqueueMessage(JText::sprintf('CM_MSGFILETYPENOTSUPPORTED', $avatarFile['type']), 'notice'); $result = false;
			}
		}
		return $result;
	}			
