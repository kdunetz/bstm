<?php
/**
 * @version $Id: joocmuser.php 133 2010-07-27 11:45:15Z sterob $
 * @package Joo!CM
...
class JoocmUser extends JUser
{
	...
	function registerAccount($data) {
		...
		if ($usersConfig->get('allowUserRegistration') == '0') {
			$this->setError(JText::_('CM_REGISTRATIONNOTALLOWED')); return false;
		}
		
		if (isset($data['id']) && $data['id'] > 0 || $this->get('id') > 0) {
			$this->setError(JText::_('CM_ALREADYREGISTERED')); return false;
		}
		
		// do a password safety check
		if(strlen($data['password']) || strlen($data['password2'])) {
			if($data['password'] != $data['password2']) {
				$this->setError(JText::_('CM_PASSWORDSDONOTMATCH')); return false;
			}
		}
	...
	function saveAccount($data) {
		...
		if (isset($data['id']) && $this->get('id') != $data['id']) {
			$this->setError(JText::_('CM_NOWAYCHANGEOTHERUSERSDATA')); return false;
		}
	...
	function saveProfile($data) {
		...
		if (!is_object($this->_joocmProfileTable)) {
			$this->setError(JText::_('CM_MSGJOOCMPROFILESNOTACTIVE')); return false;
		}
