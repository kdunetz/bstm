...
class JoocmViewEditSettings extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joocmUser->get('id') < 1) {
			$this->app->redirect(JoocmHelper::getLink('login'), JText::_('CM_MSGNOPERMISSIONEDITSETTINGS'), 'notice');
		}

		// handle page title
		$this->document->setTitle($this->joocmConfig->getConfigSettings('community_name') .' - '. JText::_('CM_MYSETTINGS'));

		// handle bread crumb
		$this->breadCrumbs->addItem(JText::_('CM_MYSETTINGS'));
		...
		$lists['show_email'] = JHTML::_('select.booleanlist', 'show_email', '', $this->joocmUser->get('show_email'), JText::_('CM_YES'), JText::_('CM_NO'));
		// build the html radio buttons for show online state
		$lists['show_online_state'] = JHTML::_('select.booleanlist', 'show_online_state', '', $this->joocmUser->get('show_online_state'), JText::_('CM_YES'), JText::_('CM_NO'));		
