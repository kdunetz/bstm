...
class JoocmViewEditProfile extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joocmUser->get('id') < 1) {
			$this->app->redirect(JoocmHelper::getLink('login'), JText::_('CM_MSGNOPERMISSIONEDITPROFILE'), 'notice');
		}
		
		if (!$this->joocmConfig->getConfigSettings('enable_profiles')) {
			$this->app->redirect(JoocmHelper::getLink('main'), JText::_('CM_MSGJOOCMPROFILESNOTACTIVE'), 'notice');
		}
		...
		// handle page title
		$this->document->setTitle($this->joocmConfig->getConfigSettings('community_name') .' - '. JText::_('CM_MYDETAILS'));
		
		// handle bread crumb
		$this->breadCrumbs->addItem(JText::_('CM_MYDETAILS'));
