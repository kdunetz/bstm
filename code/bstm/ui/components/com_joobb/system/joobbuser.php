<?php
/**
 * @version $Id: joobbuser.php 135 2010-08-13 10:03:14Z sterob $
 * @package Joo!BB
...
class JoobbUser extends JUser
{
	...
	function bind(&$array) {
		...
		if (!parent::bind($array)) {
			...
			$this->setError(JText::sprintf('BB_MSGACTIONFAILED', $this->_joobbUserTable->getError()));
			return false;
		} 
		...
		if ($isNew) {
			...
			if (!$row->load($this->id)) {
				...
				if ($db->query()) {
					...
					JError::raiseWarning(JText::sprintf('BB_MSGACTIONFAILED', $db->getError()));
					return false;					
				}
			}
		}
	...
	function load($id) {
		...
		if (!$this->_joobbUserTable->load($id)) {
			...
			if (!$db->query()) {
				...
				JError::raiseWarning(JText::sprintf('BB_MSGACTIONFAILED', $this->_joobbUserTable->getError()));
				return false;			
			}
