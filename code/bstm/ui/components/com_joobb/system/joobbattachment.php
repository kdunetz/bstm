<?php
/**
 * @version $Id: joobbattachment.php 22 2009-12-25 20:07:22Z sterob $
 * @package Joo!BB
	...
	function uploadAttachment($attachmentFile, $postId) {
		...
		if (isset($attachmentFile['name']) && $attachmentFile['name'] != '') {
			...
			if ($this->attachmentMax <= $this->getAttachmentsCount($postId)) {
				$messageQueue->addMessage(JText::sprintf('BB_MSGMAXATTACHMENTS', $attachmentFile['name'])); return false;
			}
				
			// check file types
			$fileTypes = str_replace(",", "|", $this->attachmentFileTypes);
			if (!preg_match("/$fileTypes/i", $attachmentFile['name'])) {
				$messageQueue->addMessage(JText::sprintf('BB_MSGFILETYPENOTSUPPORTED', $attachmentFile['type'])); return false;
			}
		
			// check max file size
			if ($attachmentFile['size'] > $this->attachmentMaxFileSize) {
				$messageQueue->addMessage(JText::sprintf('BB_MSGMAXIMALFILESIZE', $attachmentFile['name'], $attachmentFile['size'], $this->attachmentMaxFileSize)); return false;
			}
			...
			if (!JFile::upload($attachmentFile['tmp_name'], $attachmentPath.DS.$fileName)) {
				$messageQueue->addMessage(JText::sprintf('BB_MSGATTACHMENTUPLOADFAILED', $attachmentFile['name'])); return false;
			}
		...
		if ($attachment->load($attachmentId)) {
			...
			if ($attachment->delete()) {
				if (!unlink($attachmentFile)) {
					...
					$messageQueue->addMessage(JText::_('BB_MSGATTACHMENTFILEDELETEFAILED', $attachmentFile['name'])); return false;
				}
			} else {
			
				// error deleting attachment
				$messageQueue->addMessage(JText::_('BB_MSGATTACHMENTDELETEFAILED', $attachmentFile['name'])); return false;
			}	
		} else {
		
			// error loading attachment
			$messageQueue->addMessage(JText::_('BB_MSGATTACHMENTLOADFAILED', $attachmentFile['name'])); return false;
		}
