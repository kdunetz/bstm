<?php
/**
 * @version $Id: joobbemotionset.php 135 2010-08-13 10:03:14Z sterob $
 * @package Joo!BB
...
class JoobbEmotionSet
{
	...
	function JoobbEmotionSet($xmlFile) {
		...
		if (file_exists(JOOBB_EMOTIONS.DS.$this->name.DS.$xmlFile)) {
			...
			if ($emotionSet) {
				...
				$messageQueue->addMessage(JText::sprintf('BB_MSGXMLPARSINGERROR', JText::_('BB_EMOTIONSET'), JOOBB_EMOTIONS.DS.$this->name.DS.$xmlFile));
			}
		} else {
			$messageQueue->addMessage(JText::sprintf('BB_MSGRESSOURCENOTFOUND', JText::_('BB_EMOTIONSET'), JOOBB_EMOTIONS.DS.$this->name.DS.$xmlFile));
		}
	...
	function loadLanguage($xmlFile) {
		...
		if (is_file($langFile)) {
			...
			$messageQueue->addMessage(JText::sprintf('BB_MSGNOLANGUAGEFILE', JText::_('BB_EMOTIONSET')));
		}
	}
