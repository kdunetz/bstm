...
class JoobbViewEditPost extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->topic->id == 0) {
			$this->messageQueue->addMessage(JText::_('BB_MSGTOPICNOTSPECIFIED'));
				...
				if ($this->forum->locked || $this->topic->status == 1) {
					...
					if (!$joobbAuth->getAuth($authPostAll, $this->topic->id_forum)) {
						...
						if ($this->forum->locked) {
							...
							$this->messageQueue->addMessage(JText::_('BB_MSGTOPICLOCKED'));
						}
		...
		if (!$canPost) {
			if ($this->post->id) {
				$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGEDITPOST')));
			} else {
				$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGREPLYPOST')));
			}
