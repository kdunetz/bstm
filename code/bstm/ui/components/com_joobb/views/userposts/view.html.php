...
class JoobbViewUserPosts extends JView
{
	...
	function display($tpl = null) {
		...
		// handle page title
		$this->document->setTitle(JText::sprintf('BB_USERPOSTSTITLE', $userName, $this->joobbConfig->getBoardSettings('board_name')));
		
		// handle metadata
		$this->document->setDescription(JText::sprintf('BB_USERPOSTSTITLE', $userName, $this->joobbConfig->getBoardSettings('board_name')));
		$this->document->setMetadata('keywords', $userName .', '. JText::_('BB_POSTS'));
				
		// handle bread crumb
		$this->breadCrumbs->addBreadCrumb(JText::sprintf('BB_POSTSBY', $userName));
