<?php
/**
 * @version $Id: view.html.php 138 2010-08-16 13:42:41Z sterob $
 * @package Joo!BB
...
class JoobbViewBoard extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->showBoxLatestItems) {
			...
			switch ((int)$this->joobbConfig->getBoardSettings('latest_items_type')) {
				case 0:
					$latestItemsHeader = JText::_('BB_LATESTTOPICS');
					break;
				case 1:
					$latestItemsHeader = JText::_('BB_LATESTPOSTS');
					break;
			}
		...
		$this->assignRef('searchInputBoxText', JText::_('BB_SEARCHTHISBOARD'));
		...
		if ($this->enableFeeds) {
			switch ((int)$this->joobbConfig->getFeedSettings('feed_items_type')) {
				case 0:
					$feedText = sprintf(JText::_('BB_GETFEEDTOPICS'), $boardName);
					break;
				case 1:
					$feedText = sprintf(JText::_('BB_GETFEEDPOSTS'), $boardName);
					break;
				default:
					break;
			}
	...
	function &getOnlineUser($index = 0) {
		...
		if ($onlineUser->name) {
			...
			$onlineUser->name = JText::_('BB_GUEST');
		}
