<?php
/**
 * @version $Id: joobb.php 196 2010-11-01 09:20:31Z sterob $
 * @package Joo!BB
...
class JoobbController extends JController
{
	...
	function joobbSaveTopic() {
		...
		if ($joobbConfig->getCaptchaSettings('captcha_editpost')) {
			...
			if (md5($post['captcha_code']) != $session->get('captcha_code')) {
				...
				$messageQueue->addMessage(JText::_('BB_MSGCAPTCHACODEDONOTMATCH'));
		...
		if (isset($time)) {
			...
			if ((strtotime(gmdate("Y-m-d H:i:s")) - strtotime($time)) < $floodInterval) {
				...
				$session =& JFactory::getSession();
				$session->set('joobbPost', $post);
				$messageQueue->addMessage(JText::sprintf('BB_MSGFLOODINTERVAL', $floodInterval));
				$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=edittopic&forum='.(int)$post['id_forum'].'&topic='.$topicId.'&Itemid='.$Itemid, false)); return;
			}
		}
			...
			if ($isNew ||																					// ... we are about to create a new topic
				...
				if ($joobbForum->locked) {
					...
					if (!$joobbAuth->getAuth($authTopicAll, (int)$post['id_forum'])) {
						...
						$messageQueue->addMessage(sprintf(JText::_('BB_MSGFORUMLOCKED'), $joobbForum->name));
		...
		if (!$canPost) {
			if ($isNew) {
				...
				$messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGPOSTTOPIC')));
			} else {
				$link = JRoute::_('index.php?option=com_joobb&view=topic&topic='.$topicId.'&Itemid='.$Itemid, false);
				$messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGEDITTOPIC')));
			}
		...
		if ($attachmentResult) {
			$messageQueue->addMessage(JText::_('BB_MSGTOPICSAVED'));
	...
	function joobbSavePost() {
		...
		if ($joobbForum->load((int)$post['id_forum'])) {
			...
			if (!$joobbForum->status) {
				...
				$messageQueue->addMessage(JText::_('BB_MSGFORUMCLOSED'));
				$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='.$Itemid, false)); return;
			}
		} else {
			
			// ... otherwise leave
			$messageQueue->addMessage(JText::_('BB_MSGFORUMNOTSPECIFIED'));
			$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='.$Itemid, false)); return;
		}
		...
		if (!$joobbTopic->load((int)$post['id_topic'])) {
			...
			// ... otherwise leave...
			$messageQueue->addMessage(JText::_('BB_MSGTOPICNOTSPECIFIED'));
				...
				if ($joobbForum->locked || $joobbTopic->status == 1) {
					...
					if (!$joobbAuth->getAuth($authPostAll, $joobbForum->id)) {
						...
						if ($joobbForum->locked) {
							...
							$messageQueue->addMessage(JText::_('BB_MSGTOPICLOCKED'));
						}
		...
		if (!$canPost) {
			if ($isNew) {
				$messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGREPLYPOST')).$joobbForum->id);
			} else {
				$messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGEDITPOST')));
			}
		...
		if (!$joobbPost->subject) {
			...
			$joobbPost->subject = JText::_('BB_REPLYSHORT').' '.$joobbTopicPost->subject;		
		}
		...
		if ($attachmentResult) {
			$messageQueue->addMessage(JText::_('BB_MSGPOSTSAVED'));
	...
	function joobbDeleteTopic() {
		...
		if ($joobbTopic->load($topicId)) {
			...
			if ($joobbPost->load($joobbTopic->id_first_post)) {
				...
				$msg = JText::_('BB_MSGPOSTNOTFOUND');
				$link = JRoute::_('index.php?option=com_joobb&view=board&Itemid='.$Itemid, false);
			}
		} else {
			$msg = JText::_('BB_MSGTOPICNOTFOUND');
			$link = JRoute::_('index.php?option=com_joobb&view=board&Itemid='.$Itemid, false);			
		}
		...
		if ($joobbPost->load($postId)) {
			...
			if ($joobbAuth->getAuth('auth_delete', $joobbPost->id_forum)) {
				...
				if ($joobbUser->get('id') == $joobbPost->id_user && $joobbPost->id_user != 0 || 
					...
					$msg = JText::_('BB_MSGPOSTDELETED');
					$link = JRoute::_('index.php?option=com_joobb&view=topic&topic='.$topicId.'&Itemid='.$Itemid, false);
				} else {
					$msg = JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGDELETEPOST');
	...
	function joobbReportPost() {
		...
		if ($joobbPost->load(JRequest::getVar('post', 0, '', 'int'))) {
			...
			if ($joobbAuth->getAuth('auth_reportpost', $joobbPost->id_forum)) {
				...
				$messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGREPORT')));
			}
		} else {
			$messageQueue->addMessage(JText::_('BB_MSGPOSTTOREPORTNOTFOUND'));
		}
			...
			if ($joobbAuth->getAuth('auth_lock', $joobbTopic->id_forum)) {
				...
				if ($joobbPost->load($joobbTopic->id_first_post)) {
					...
					if ($joobbUser->get('id') == $joobbPost->id_user && $joobbPost->id_user != 0 || 
						...
						$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGLOCKTOPIC'));
					}
				}
			} else {
				$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGLOCKTOPIC'));
			...
			if ($joobbAuth->getAuth('auth_lock', $joobbTopic->id_forum)) {
				...
				if ($joobbPost->load($joobbTopic->id_first_post)) {
					...
					if ($joobbUser->get('id') == $joobbPost->id_user && $joobbPost->id_user != 0 || 
						...
						$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGUNLOCKTOPIC'));
					}
				}
			} else {
				$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGUNLOCKTOPIC'));
	...
	function joobbMoveTopic() {
		...
		if (!$joobbForumMoveTo->load($forumId)) {
			$messageQueue->addMessage(JText::_('BB_MSGFORUMNOTFOUND'));
			$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='. $Itemid, false)); return;	
		}
		...
		if ($joobbForumMoveTo->auth_post <= $joobbUser->getRole($forumId)) {
			...
			$joobbTopic =& JTable::getInstance('JoobbTopic');
			if (!$joobbTopic->load($topicId)) {
				$messageQueue->addMessage(JText::_('BB_MSGTOPICNOTFOUND'));
				$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='. $Itemid, false)); return;
			}
					...
					if (!$db->query()) {
						...
						if (!$joobbForum->store()) {
							...
							if (!$joobbForumMoveTo->store()) {
								...
								$messageQueue->addMessage(JText::_('BB_MSGTOPICMOVED'));	
							}
						}	
					}
				}
			} else {
				$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGMOVETOPIC'));
			}
		} else {
			$messageQueue->addMessage(JText::sprintf('BB_MSGNOPERMISSION', 'BB_MSGMOVETOPIC'));
		}
	...
	function joobbSubscribeTopic() {
		...
		if ($joobbUser->get('id') < 1) {
			$messageQueue->addMessage(JText::_('BB_MSGSUBSCRIPTIONNOTALLOWED'));
			$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='. $Itemid, false)); return;
		}
	...
	function joobbUnsubscribeTopic() {
		...
		if ($joobbUser->get('id') < 1) {
			$messageQueue->addMessage(JText::_('BB_MSGUNSUBSCRIPTIONNOTALLOWED'));
			$this->setRedirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='. $Itemid, false)); return;
		}
	...
	function joobbSaveSettings() {
		...
		if ($joobbUser->get('id') < 1) {
			$msg = JText::_('BB_MSGNOPERMISSIONEDITSETTINGS');	
		} else {
			$msg = JText::_('BB_MSGSETTINGSSAVED');
