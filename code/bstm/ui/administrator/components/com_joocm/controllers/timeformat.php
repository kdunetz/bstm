...
class ControllerTimeFormat extends JController
{
	...
	function editTimeFormat() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_timeformat_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_TIMEFORMAT'), $row->name, $editingUser->name)); return;
		}
	...
	function saveTimeFormat() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_TIMEFORMAT'), $row->name);
	...
	function deleteTimeFormat() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_TIMEFORMAT'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_TIMEFORMATS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_TIMEFORMAT'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function changeTimeFormatPublishState($state = 0) {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmTimeFormat'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_TIMEFORMAT'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_TIMEFORMATS'), '');
			}
	...
	function defaultTimeFormat() {
		...
		if (count($cid)) {
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_CONFIG'), JText::_('CM_DEFAULT')); $msgType = 'notice';
		}
