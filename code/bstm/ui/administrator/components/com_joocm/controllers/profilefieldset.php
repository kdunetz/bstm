...
class ControllerProfileFieldSet extends JController
{
	...
	function editProfileFieldSet() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefieldset_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_PROFILEFIELDSET'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveProfileFieldSet() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_PROFILEFIELDSET'), $row->name);
	...
	function deleteProfileFieldSet() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDSET'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDSETS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_PROFILEFIELDSET'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function orderProfileFieldSet() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYREORDERED', JText::_('CM_PROFILEFIELDSET'), $row->name);
	...
	function saveProfileFieldSetOrder() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVEDORDER', JText::_('CM_PROFILEFIELDSET'));
		$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefieldset_view', $msg);
	}
	...
	function changeProfileFieldSetPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmProfileFieldSet'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDSET'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDSETS'), '');
			}
