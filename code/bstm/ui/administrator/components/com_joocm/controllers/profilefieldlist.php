<?php
/**
 * @version $Id: profilefieldlist.php 132 2010-07-26 11:50:33Z sterob $
 * @package Joo!CM
...
class ControllerProfileFieldList extends JController
{
	...
	function editProfileFieldList() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefieldlist_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_PROFILEFIELDLIST'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveProfileFieldList() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_PROFILEFIELDLIST'), $row->name);
	...
	function deleteProfileFieldList() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDLIST'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDLISTS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_PROFILEFIELDLIST'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function changeProfileFieldListPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmProfileFieldList'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDLIST'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDLISTS'), '');
			}
