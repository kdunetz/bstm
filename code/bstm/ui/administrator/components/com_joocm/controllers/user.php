...
class ControllerUser extends JController
{
	...
	function showUsers() {
		...
		$query = "SELECT name AS value, name AS text"
				...
				;
		$db->setQuery($query);
		$types[] = JHTML::_('select.option', '0', '- '.JText::_('CM_SELECTGROUP').' -', 'value', 'text');
		...
		$logged[] = JHTML::_('select.option', 0, '- '.JText::_('CM_SELECTLOGSTATUS').' -', 'value', 'text');
		$logged[] = JHTML::_('select.option', 1, '- '.JText::_('CM_LOGGEDIN').' -', 'value', 'text');
	...
	function saveUser() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_USER'), $joocmUser->get('name'));
	...
	function deleteUser() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_USER'), '');
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_USERS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_USER'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function logoutUser() {
		...
		if (count($cid)) {
			...
			if ($db->query()) {
				...
				if (count($cid) == 1) {
					$joocmUser =& JoocmUser::getInstance($cid[0]);
					$msg = JText::sprintf('CM_MSGSUCCESSFULLYLOGGEDOUT', JText::_('CM_USER'), $joocmUser->get('name'));
				} else {
					$msg = JText::sprintf('CM_MSGSUCCESSFULLYLOGGEDOUT', JText::_('CM_USERS'), '');
				}
			} else {
				$msg = $db->getErrorMsg(); $msgType = 'error';
			}
		} else {
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_USER'), JText::_('CM_LOGOUT')); $msgType = 'notice';
		}
