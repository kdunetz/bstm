<?php
/**
 * @version $Id: interface.php 132 2010-07-26 11:50:33Z sterob $
 * @package Joo!CM
		...
		<div id="submenu-box">
			...
			<div class="m">
				<ul id="submenu"><li>
					...
					<a <?php echo $class; ?> href="index.php?option=com_joocm&task=joocm_interface_view"><?php echo JText::_('CM_INTERFACES'); ?></a>
				</li><li>
					<?php $class = ($task == 'joocm_link_view') ? 'class="active"' : ''; ?>
					<a <?php echo $class; ?> href="index.php?option=com_joocm&task=joocm_link_view"><?php echo JText::_('CM_LINKS'); ?></a>
				</li></ul>
				<div class="clr"></div>
			</div>
		...
		<form action="index.php?option=com_joocm" method="post" name="adminForm">
			...
			<thead><tr>
                ...
                <th nowrap="nowrap" width="82%">
                    <?php echo JHTML::_('grid.sort', 'CM_NAME', 'i.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                </th>
                <th nowrap="nowrap" width="5%">
                    <?php echo JHTML::_('grid.sort', 'CM_CLIENT', 'i.client', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                </th>
                <th nowrap="nowrap" width="5%">
                    <?php echo JHTML::_('grid.sort', 'CM_SYSTEM', 'i.system', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                </th>
                <th nowrap="nowrap" width="8%">
                    <?php echo JHTML::_('grid.sort', 'CM_ORDER', 'i.ordering', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                    <?php echo JHTML::_('grid.order', $rows, 'filesave.png', 'joocm_interface_saveorder' ); ?>
                </th>
                <th nowrap="nowrap" width="1%">
                    <?php echo JHTML::_('grid.sort', 'CM_PUBLISHED', 'i.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                </th>
                <th nowrap="nowrap" width="1%">
                    <?php echo JHTML::_('grid.sort', 'CM_ID', 'i.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_interface_view'); ?>
                </th>
			</tr></thead>
			<tfoot>
				<td colspan="8">
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				$client = $row->client ? JText::_('CM_BACKEND') : JText::_('CM_FRONTEND');
				$system = $row->system ? JText::_('CM_YES') : JText::_('CM_NO');
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td class="order">
						<span><?php echo $pageNav->orderUpIcon($i, ($row->client == @$rows[$i-1]->client), 'joocm_interface_orderup', 'CM_ORDERUP', true); ?></span>
						<span><?php echo $pageNav->orderDownIcon($i, $n, ($row->client == @$rows[$i+1]->client), 'joocm_interface_orderdown', 'CM_ORDERDOWN', true); ?></span>
	...
	function editInterface(&$row, &$lists) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_NAME'), JText::_('CM_INTERFACE')); ?>");
			} else {
		...
		<form action="index.php" method="post" name="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_INTERFACEDETAILS'); ?></legend>		
					<table class="admintable" cellspacing="1"><tr>
						...
						<td class="key">
							<label for="com_icon">
								<?php echo JText::_('CM_COMPONENTICON'); ?>
							</label>
						</td><td>
						...
						<td class="key">
							<label for="client">
								<?php echo JText::_('CM_CLIENT'); ?>
							</label>
						</td>
						<td><?php echo $lists['client']; ?></td>
					</tr><tr>
						<td class="key">
							<label for="show_restriction" class="hasTip" title="<?php echo JText::_('CM_SHOWRESTRICTION') .'::'. JText::_('CM_INTERFACESHOWRESTRICTIONDESC'); ?>">
								<?php echo JText::_('CM_SHOWRESTRICTION'); ?>
							</label>
						</td>
						<td><?php echo $lists['show_restriction']; ?></td>
					</tr><tr>
						<td class="key">
							<label for="published" title="<?php echo JText::_('CM_PUBLISHED') .'::'. JText::_('CM_INTERFACEPUBLISHEDDESC'); ?>">
