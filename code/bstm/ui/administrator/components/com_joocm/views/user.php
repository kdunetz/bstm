<?php
/**
 * @version $Id: user.php 132 2010-07-26 11:50:33Z sterob $
 * @package Joo!CM
...
class ViewUser {
	...
	function showUsers(&$rows, &$pageNav, &$lists) {
		...
		<div id="submenu-box">
			...
			<div class="m">
				<ul id="submenu"><li>
					...
					<a <?php echo $class; ?> href="index.php?option=com_joocm&task=joocm_user_view"><?php echo JText::_('CM_USERS'); ?></a>
				</li><li>
					<?php $class = ($task == 'joocm_avatar_view') ? 'class="active"' : ''; ?>
					<a <?php echo $class; ?> href="index.php?option=com_joocm&task=joocm_avatar_view"><?php echo JText::_('CM_AVATARS'); ?></a>
				</li></ul>
				<div class="clr"></div>
			</div>
		...
		<form action="index.php?option=com_joocm" method="post" name="adminForm">
			<table><tr>
				<td width="100%">
					<?php echo JText::_('CM_FILTER'); ?>:
					<input type="text" name="search" id="search" value="<?php echo $lists['search'];?>" class="text_area" onchange="document.adminForm.submit();" />
					<button onclick="this.form.submit();"><?php echo JText::_('CM_GO'); ?></button>
					<button onclick="getElementById('search').value='';this.form.submit();"><?php echo JText::_('CM_RESET'); ?></button>
				</td>
				<td nowrap="nowrap">
					<?php echo $lists['type'];?>
			...
			<table class="adminlist" cellpadding="1">
				<thead><tr>
					...
					<th class="title">
						<?php echo JHTML::_('grid.sort', 'CM_NAME', 'a.name', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="15%" class="title" >
						<?php echo JHTML::_('grid.sort', 'CM_USERNAME', 'a.username', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="5%" class="title" nowrap="nowrap">
						<?php echo JText::_('CM_LOGGEDIN'); ?>
					</th>
					<th width="5%" class="title" nowrap="nowrap">
						<?php echo JHTML::_('grid.sort', 'CM_ENABLED', 'a.block', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="15%" class="title">
						<?php echo JHTML::_('grid.sort', 'CM_GROUP', 'a.groupname', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="15%" class="title">
						<?php echo JHTML::_('grid.sort', 'CM_EMAIL', 'a.email', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="10%" class="title">
						<?php echo JHTML::_('grid.sort', 'CM_LASTVISIT', 'a.lastvisitDate', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
					<th width="1%" class="title" nowrap="nowrap">
						<?php echo JHTML::_('grid.sort', 'CM_ID', 'a.id', @$lists['order_Dir'], @$lists['order'], 'joocm_user_view'); ?>
					</th>
				</tr></thead>
				<tfoot>
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				$alt 	= $row->block ? JText::_('CM_ENABLED') : JText::_('CM_BLOCKED');
	...
	function editUser(&$user, &$lists) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_NAME'), JText::_('CM_USER')); ?>");
			} else if (form.username.value == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_USERNAME'), JText::_('CM_USER')); ?>");
			} else if (r.exec(form.username.value) || form.username.value.length < 3) {
				alert("<?php echo JText::_('WARNLOGININVALID', true); ?>");
			} else if (trim(form.email.value) == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_EMAILADDRESS'), JText::_('CM_USER')); ?>");
			} else {
		...
		<form action="index.php" method="post" name="adminForm" id="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					...
					<table class="admintable" cellspacing="1"><tr>
						...
						<td class="key">
							<label for="password2">
								<?php echo JText::_('CM_VERIFYPASSWORD'); ?>
							</label>
						</td><td><?php
						if(!$user->get('password')) : ?>
					...
					if($user->get('id')) { ?>
					<tr>
						<td valign="top" class="key"><?php echo JText::_('CM_GROUP'); ?></td>
						<td><?php echo $user->get('usertype'); ?></td>
					</tr><?php
					} ?>
					<tr>
						<td class="key"><?php echo JText::_('CM_BLOCKUSER'); ?></td>
						<td><?php echo $lists['block']; ?></td>
					</tr><tr>
						<td class="key"><?php echo JText::_('CM_RECEIVESYSTEMEMAILS'); ?></td>
						<td><?php echo $lists['sendEmail']; ?></td>
					</tr><?php
					if($user->get('id')) { ?>
					<tr>
						<td class="key"><?php echo JText::_('CM_REGISTERDATE'); ?></td>
						<td><?php echo $user->get('registerDate'); ?></td>
					</tr><tr>
						<td class="key"><?php echo JText::_('CM_LASTVISITDATE'); ?></td>
				...
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_PARAMETERS'); ?></legend>
			...
			<div class="col width-50">
				<fieldset class="adminform">
					...
					<table class="admintable" cellspacing="1"><tr>
						<td class="key">
							<label for="system_emails"><?php echo JText::_('CM_RECEIVESYSTEMEMAILS'); ?></label>
						</td>
						<td><?php echo $lists['system_emails']; ?></td>
					</tr><tr>
						<td class="key">
							<label for="system_emails"><?php echo JText::_('CM_AGREEDTERMS'); ?></label>
						</td>
						<td><?php echo $lists['agreed_terms']; ?></td>
					</tr></table>
				</fieldset>
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_PREFERENCES'); ?></legend>
						<table class="admintable" cellspacing="1"><tr>
							<td class="key"><?php echo JText::_('CM_SHOWEMAIL'); ?></td>
							<td><?php echo $lists['show_email']; ?></td>
						</tr><tr>
							<td class="key"><?php echo JText::_('CM_SHOWONLINESTATE'); ?></td>
							<td><?php echo $lists['show_online_state']; ?></td>
						</tr><tr>
						<td class="key">
							<label for="time_format"><?php echo JText::_('CM_TIMEFORMAT'); ?></label>
						</td>
						<td><?php echo $lists['timeformats']; ?></td>
					</tr></table>
				</fieldset>
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_SIGNATURE'); ?></legend>
					<table class="admintable" cellspacing="1"><tr>
						<td class="key">
							<label for="signature">
								<?php echo JText::_('CM_SIGNATURE'); ?>
							</label>
						</td>
			...
			<div class="col100">
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_PROFILE'); ?></legend><?php
	...
	function _getProfileFieldSets($fieldsets, $fields) {
		...
		if (count($fieldsets)) {
			...
			echo JText::_('CM_MSGJOOCMPROFILESNOTACTIVE');
		}		
	}
}
?>