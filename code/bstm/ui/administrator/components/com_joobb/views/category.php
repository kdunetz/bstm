<?php
/**
 * @version $Id: category.php 190 2010-10-21 17:56:57Z sterob $
 * @package Joo!BB
		...
		function submitbutton(pressbutton) {
			if (pressbutton == 'joobb_category_delete') {
				if (document.adminForm.boxchecked.value > 0) {
					var confirmed = confirm("<?php echo JText::_('BB_MSGWARNINGDELETECATEGORY'); ?>");
					if (!confirmed){
		...
		<form action="index.php?option=com_joobb" method="post" name="adminForm">
			...
			<thead><tr>
				...
				<th nowrap="nowrap" width="88%">
					<?php echo JHTML::_('grid.sort', 'BB_CATEGORY', 'c.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joobb_category_view'); ?>
				</th>
				<th nowrap="nowrap" width="1%">
					<?php echo JHTML::_('grid.sort', 'BB_PUBLISHED', 'c.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joobb_category_view'); ?>
				</th>
				<th nowrap="nowrap" width="10%">
					<?php echo JHTML::_('grid.sort', 'BB_ORDER', 'c.ordering', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joobb_category_view'); ?>
					<?php echo JHTML::_('grid.order', $rows, 'filesave.png', 'joobb_category_saveorder' ); ?>
				</th>																							
				<th nowrap="nowrap" width="1%">
					<?php echo JHTML::_('grid.sort', 'BB_ID', 'c.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joobb_category_view'); ?>
				</th>
			</tr></thead>
			<tfoot><tr>
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td class="order">
						<span><?php echo $pageNav->orderUpIcon($i, true, 'joobb_category_orderup', 'BB_ORDERUP', true); ?></span>
						<span><?php echo $pageNav->orderDownIcon($i, $n, true, 'joobb_category_orderdown', 'BB_ORDERDOWN', true); ?></span>
	...
	function editCategory(&$row, &$lists) { ?>
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('BB_MSGFIELDREQUIRED', JText::_('BB_NAME'), JText::_('BB_CATEGORY')); ?>");
			} else {
				submitform(pressbutton);
			}
		}
		...
		<form action="index.php" method="post" name="adminForm" id="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend>
						<?php echo JText::_('BB_CATEGORYDETAILS'); ?>
					</legend>
					<table class="admintable" cellspacing="1"><tr>
						<td class="key">
							<label for="name" class="hasTip" title="<?php echo JText::_('BB_NAME') .'::'. JText::_('BB_CATEGORYNAMEDESC'); ?>">
						...
						<td class="key">
							<label for="published" class="hasTip" title="<?php echo JText::_('BB_PUBLISHED') .'::'. JText::_('BB_CATEGORYPUBLISHEDDESC'); ?>">
