...
class ControllerUserSync extends JController
{
	...
	function showUserSync() {
		...
		$joomlaGroup = array();
		$joomlaGroup[] = JHTML::_('select.option', 0, JText::_('BB_SELECTJOOMLAGROUP'));
		$joomlaGroup[] = JHTML::_('select.option', 1, JText::_('BB_ALLUSERS'));
		$joomlaGroup[] = JHTML::_('select.option', 18, JText::_('BB_REGISTERED'));
		$joomlaGroup[] = JHTML::_('select.option', 19, JText::_('BB_AUTHOR'));
		$joomlaGroup[] = JHTML::_('select.option', 20, JText::_('BB_EDITOR'));
		$joomlaGroup[] = JHTML::_('select.option', 21, JText::_('BB_PUBLISHER'));
		$joomlaGroup[] = JHTML::_('select.option', 23, JText::_('BB_MANAGER'));
		$joomlaGroup[] = JHTML::_('select.option', 24, JText::_('BB_ADMINISTRATOR'));
		$joomlaGroup[] = JHTML::_('select.option', 25, JText::_('BB_SUPERADMINISTRATOR'));
	...
	function performUserSync() {
		...
		if (!$joomlaGroup) {
			$this->setRedirect('index.php?option=com_joobb&task=joobb_usersync_view', JText::_('BB_MSGSELECTJOOMLAGROUP'), 'notice'); return;
		...
		if (count($joomlaUsers)) {
			...
			$msg = JText::sprintf('BB_MSGSUCCESSFULLYSYNCHRONIZED', JText::_('BB_USERS'));
		} else {
			$msg = JText::_('BB_MSGNOUSERSTOSYNCHRONIZE'); $msgType = 'notice';
		}
