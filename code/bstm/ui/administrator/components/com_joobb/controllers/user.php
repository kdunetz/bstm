...
class ControllerUser extends JController
{
	...
	function showUsers() {
		...
		$query = "SELECT name AS value, name AS text"
				...
				;
		$db->setQuery($query);
		$types[] = JHTML::_('select.option', '0', '- '.JText::_('BB_SELECTGROUP').' -', 'value', 'text');
		...
		$logged[] = JHTML::_('select.option', 0, '- '.JText::_('BB_SELECTLOGSTATUS').' -', 'value', 'text');
		$logged[] = JHTML::_('select.option', 1, '- '.JText::_('BB_LOGGEDIN').' -', 'value', 'text');
	...
	function saveUser() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_USER'), $joobbUser->get('name'));
	...
	function deleteUser() {
		...
		if (count($cid) < 1) {
			$this->setRedirect('index.php?option=com_joobb&task=joobb_user_view', JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_USER'), JText::_('BB_DELETE')), 'notice'); return;
		}
	...
	function changeUserBlock() {
		...
		if (count($cid) < 1) {
			...
			$this->setRedirect('index.php?option=com_joobb&task=joobb_user_view', JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_USER'), $action), 'notice'); return;
		}
	...
	function logoutUser() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$joobbUser =& JoobbUser::getInstance($cid[0]);
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYLOGGEDOUT', JText::_('BB_USER'), $joobbUser->get('name'));
			} else {
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYLOGGEDOUT', JText::_('BB_USERS'), '');
			}
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_USER'), JText::_('BB_LOGOUT')); $msgType = 'notice';
		}
