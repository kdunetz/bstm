...
class ControllerCategory extends JController
{
	...
	function editCategory() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joobb&task=joobb_category_view', JText::sprintf('BB_MSGBEINGEDITTED', JText::_('BB_CATEGORY'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveCategory() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_CATEGORY'), $row->name);
	...
	function deleteCategory() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_CATEGORY'), $row->name);
			} else {
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_CATEGORIES'), '');
			}
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_CATEGORY'), JText::_('BB_DELETE')); $msgType = 'notice';
		}
	...
	function orderCategory() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYREORDERED', JText::_('BB_CATEGORY'), $row->name);
	...
	function saveCategoryOrder() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVEDORDER', JText::_('BB_CATEGORY'));
		$this->setRedirect('index.php?option=com_joobb&task=joobb_category_view', $msg);
	}
	...
	function changeCategoryPublishState() {
		...
		if (count($cid)) {
			$msgText = ($state == 1) ? 'BB_MSGSUCCESSFULLYPUBLISHED' : 'BB_MSGSUCCESSFULLYUNPUBLISHED';
			
			// are there one or more rows to publish/unpublish?
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoobbCategory'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('BB_CATEGORY'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('BB_CATEGORIES'), '');
			}
