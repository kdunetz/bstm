<?php
/**
 * @version $Id: forum.php 194 2010-10-25 21:33:22Z sterob $
 * @package Joo!BB
...
class ControllerForum extends JController
{
	...
	function editForum() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joobb&task=joobb_forum_view', JText::sprintf('BB_MSGBEINGEDITTED', JText::_('BB_FORUM'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveForum() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_FORUM'), $row->name);
	...
	function deleteForum() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_FORUM'), $row->name);
			} else {
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_FORUMS'), '');
			}
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_FORUM'), JText::_('BB_DELETE')); $msgType = 'notice';
		}
	...
	function orderForum() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYREORDERED', JText::_('BB_FORUM'), $row->name);
	...
	function saveForumOrder() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVEDORDER', JText::_('BB_FORUM'));
		$this->setRedirect('index.php?option=com_joobb&task=joobb_forum_view', $msg);
	}
	...
	function changeForumPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoobbForum'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('BB_FORUM'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('BB_FORUMS'), '');
			}
