...
<?php
     ...
     //$objects_query = tep_db_query("select *, datediff(end_date,curdate()) days_left from coupon WHERE (user_id = " . $user_id . " OR user_id IS NULL OR user_id = 0)  ORDER BY end_date");
     $objects_query = tep_db_query("select *, datediff(end_date,curdate()) days_left from coupon WHERE datediff(end_date,curdate()) < 10 AND datediff(end_date,curdate()) >= 0 AND (user_id = " . $user_id . " OR user_id IS NULL OR user_id = 0) ORDER BY end_date");
