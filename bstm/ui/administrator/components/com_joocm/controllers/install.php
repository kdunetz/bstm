<?php
/**
 * @version $Id: install.php 160 2010-09-03 15:52:07Z sterob $
 * @package Joo!CM
...
class ControllerInstall extends JController
{
	...
	function performInstall() {
		...
		if ($joocmInstaller->executeSQLFile(JOOCM_ADMINBASEPATH.DS.'uninstall.joocm.sql')) {
			...
			if ($joocmInstaller->executeSQLFile(JOOCM_ADMINBASEPATH.DS.'install.joocm.sql')) {
				$app->enqueueMessage(JText::_('CM_MSGINSTALLSUCCESS'));
			} else {
				$app->enqueueMessage($db->getErrorMsg(), 'error');
				$app->enqueueMessage(JText::_('CM_MSGINSTALLFAILED'), 'error');
			}
		} else {
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			$app->enqueueMessage(JText::_('CM_MSGINSTALLFAILED'), 'error');
	...
	function performUpdate() {
		...
		// make sure the update table is available
		if (!$joocmInstaller->executeSQLFile(JOOCM_ADMINBASEPATH.DS.'updates'.DS.'update.sql')) {
		
			// get the db error...
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			
			// ... and redirect with component update error message...
			$app->redirect('index.php?option=com_joocm&task=joocm_install_view', JText::_('CM_MSGUPDATEFAILED'), 'error');
		}
...
//$app->enqueueMessage($query, 'error');
		...
		if ($status == 0) {
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			$app->enqueueMessage(JText::_('CM_MSGUPDATEFAILED'), 'error');
		} else {
			$app->enqueueMessage(JText::_('CM_MSGUPDATESUCCESS'));
		}
