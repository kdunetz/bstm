...
class ControllerProfileFieldListValue extends JController
{
	...
	function showProfileFieldListValues() {
		...
		$query = "SELECT l.*"
				...
				;
		$db->setQuery($query);
		$profileFieldLists[] = JHTML::_('select.option', '0', '- '.JText::_('CM_SELECTPROFILEFIELDLIST').' -', 'id', 'name');
	...
	function editProfileFieldListValue() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefieldlistvalue_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_PROFILEFIELDLISTVALUE'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveProfileFieldListValue() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_PROFILEFIELDLISTVALUE'), $row->name);
	...
	function deleteProfileFieldListValue() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDLISTVALUE'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDLISTVALUES'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_PROFILEFIELDLISTVALUE'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function orderProfileFieldListValue() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYREORDERED', JText::_('CM_PROFILEFIELDLISTVALUE'), $row->name);
	...
	function saveProfileFieldListValueOrder() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVEDORDER', JText::_('CM_PROFILEFIELDLISTVALUE'));
		$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefieldlistvalue_view', $msg);
	}
	...
	function changeProfileFieldListValueState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmProfileFieldListValue'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDLISTVALUE'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDLISTVALUES'), '');
			}
