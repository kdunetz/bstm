...
class ControllerAvatar extends JController
{
	...
	function editAvatar() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_avatar_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_AVATAR'), $row->terms, $editingUser->name)); return;
		}
	...
	function saveAvatar() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_AVATAR'), $row->terms);
	...
	function deleteAvatar() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_AVATARS'), $row->terms);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_AVATAR'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_AVATAR'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function changeAvatarPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmAvatar'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_AVATARS'), $row->terms);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_AVATAR'), '');
			}
