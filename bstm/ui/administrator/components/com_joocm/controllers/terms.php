...
class ControllerTerms extends JController
{
	...
	function editTerms() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_terms_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_TERMS'), $row->terms, $editingUser->name), 'notice'); return;
		}
	...
	function saveTerms() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_TERMS'), $row->terms);
	...
	function deleteTerms() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_TERMS'), $row->terms);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_TERMS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_TERMS'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function changeTermsPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmTerms'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_TERMS'), $row->terms);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_TERMS'), '');
			}
