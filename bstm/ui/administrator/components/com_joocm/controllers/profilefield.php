...
class ControllerProfileField extends JController
{
	...
	function editProfileField() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefield_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_PROFILEFIELD'), $row->name, $editingUser->name), 'notice'); return;
		}
...
//		$elements[] = JHTML::makeOption(2, JText::_('CheckBox'));
	...
	function saveProfileField() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_PROFILEFIELD'), $row->name);
	...
	function deleteProfileField() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELD'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_PROFILEFIELDS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_PROFILEFIELD'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function orderProfileField() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYREORDERED', JText::_('CM_PROFILEFIELD'), $row->name);
	...
	function saveProfileFieldOrder() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVEDORDER', JText::_('CM_PROFILEFIELD'));
		$this->setRedirect('index.php?option=com_joocm&task=joocm_profilefield_view', $msg);
	}
	...
	function changeProfileFieldPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmProfileField'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELD'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_PROFILEFIELDS'), '');
			}
