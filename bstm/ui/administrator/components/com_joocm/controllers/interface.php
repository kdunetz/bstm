...
class ControllerInterface extends JController
{
	...
	function showInterfaces() {
		...
		$client[] = JHTML::_('select.option', '-1', '- '.JText::_('CM_SELECTINTERFACECLIENT').' -', 'value', 'text');
		$client[] = JHTML::_('select.option', '0', JText::_('CM_FRONTEND'), 'value', 'text');
		$client[] = JHTML::_('select.option', '1', JText::_('CM_BACKEND'), 'value', 'text');
		...
		$types[] = JHTML::_('select.option', '-1', '- '.JText::_('CM_SELECTINTERFACETYPE').' -', 'value', 'text');
		$types[] = JHTML::_('select.option', '0', JText::_('CM_NOSYSTEM'), 'value', 'text');
		$types[] = JHTML::_('select.option', '1', JText::_('CM_SYSTEM'), 'value', 'text');
	...
	function editInterface() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_interface_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_INTERFACE'), $row->name, $editingUser->name)); return;
		}
		...
		$clients = array();
		$clients[] = JHTML::_('select.option', 0, JText::_('CM_FRONTEND'));
		$clients[] = JHTML::_('select.option', 1, JText::_('CM_BACKEND'));

		$lists['client'] = JHTML::_('select.radiolist',  $clients, 'client', '', 'value', 'text', $row->client);
		
		$restrictions = array();
		$restrictions[] = JHTML::_('select.option', 0, JText::_('CM_BOTH'));
		$restrictions[] = JHTML::_('select.option', 1, JText::_('CM_OFFLINEUSERS'));
		$restrictions[] = JHTML::_('select.option', 2, JText::_('CM_ONLINEUSERS'));
	...
	function saveInterface() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_INTERFACE'), $row->name);
	...
	function deleteInterface() {
		...
		if (count($cid)) {
			...
			foreach ($cid as $id) {
				...
				if ($row->system) {
					$app->enqueueMessage(JText::sprintf('CM_MSGSYSTEMINTERFACENOTDELETED', $row->name), 'error');
				} else {
					$row->delete();
					$deletedItems++;
				}
			...
			if ($deletedItems == 1) {
				$app->enqueueMessage(JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_INTERFACE')));
			} else if ($deletedItems > 1) {
				$app->enqueueMessage(JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_INTERFACES')));
			}
		} else {
			$app->enqueueMessage(JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_INTERFACE'), JText::_('CM_DELETE')), 'notice');
		}
	...
	function orderInterface() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYREORDERED', JText::_('CM_INTERFACE'), JText::_($row->name));
	...
	function saveInterfaceOrder() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVEDORDER', JText::_('CM_INTERFACE'));
		$this->setRedirect('index.php?option=com_joocm&task=joocm_interface_view', $msg);
	}
	...
	function changeInterfacePublishState($state = 0) {
		...
		if (count($cid)) {
			$msgText = ($state == 1) ? 'CM_MSGSUCCESSFULLYPUBLISHED' : 'CM_MSGSUCCESSFULLYUNPUBLISHED';
			
			// are there one or more rows to publish/unpublish?
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmInterface'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_INTERFACE'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_INTERFACES'), '');
			}
