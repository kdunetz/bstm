...
class ControllerUserSync extends JController
{
	...
	function showUserSync() {
		...
		$joomlaGroup = array();
		$joomlaGroup[] = JHTML::_('select.option', 0, JText::_('CM_SELECTJOOMLAGROUP'));
		$joomlaGroup[] = JHTML::_('select.option', 1, JText::_('CM_ALLUSERS'));
		$joomlaGroup[] = JHTML::_('select.option', 18, JText::_('CM_REGISTERED'));
		$joomlaGroup[] = JHTML::_('select.option', 19, JText::_('CM_AUTHOR'));
		$joomlaGroup[] = JHTML::_('select.option', 20, JText::_('CM_EDITOR'));
		$joomlaGroup[] = JHTML::_('select.option', 21, JText::_('CM_PUBLISHER'));
		$joomlaGroup[] = JHTML::_('select.option', 23, JText::_('CM_MANAGER'));
		$joomlaGroup[] = JHTML::_('select.option', 24, JText::_('CM_ADMINISTRATOR'));
		$joomlaGroup[] = JHTML::_('select.option', 25, JText::_('CM_SUPERADMINISTRATOR'));
	...
	function performUserSync() {
		...
		if (!$joomlaGroup) {
			$this->setRedirect('index.php?option=com_joocm&task=joocm_usersync_view', JText::_('CM_MSGSELECTJOOMLAGROUPSYNC'), 'notice'); return;
		...
		if (count($joomlaUsers)) {
			...
			foreach ($joomlaUsers as $joomlaUser) {
				...
				if ($userExists) {		
					...
					$query = "INSERT INTO #__joocm_users (id, system_emails, show_email, show_online_state, time_format) VALUES"
							. "\n (".$joomlaUser->id.",$systemEmails,$showEmail,$showOnlineState,'$timeFormat')"
							;
					$db->setQuery($query);
			...
			$msg = JText::sprintf('CM_MSGSUCCESSFULLYSYNCHRONIZED', JText::_('CM_USERS'));
		} else {
			$msg = JText::_('CM_MSGNOUSERSTOSYNCHRONIZE'); $msgType = 'notice';
		}
