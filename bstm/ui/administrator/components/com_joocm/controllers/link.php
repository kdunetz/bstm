...
class ControllerLink extends JController
{
	...
	function editLink() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joocm&task=joocm_link_view', JText::sprintf('CM_MSGBEINGEDITTED', JText::_('CM_LINK'), $row->name, $editingUser->name), 'notice'); return;
		}
	...
	function saveLink() {
		...
		$msg = JText::sprintf('CM_MSGSUCCESSFULLYSAVED', JText::_('CM_LINK'), $row->name);
	...
	function deleteLink() {
		...
		if (count($cid)) {
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_LINK'), $row->name);
			} else {
				$msg = JText::sprintf('CM_MSGSUCCESSFULLYDELETED', JText::_('CM_LINKS'), '');
			}
			...
			$msg = JText::sprintf('CM_MSGNOSELECTION', JText::_('CM_LINK'), JText::_('CM_DELETE')); $msgType = 'notice';
		}
	...
	function changeLinkPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoocmLink'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('CM_LINK'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('CM_LINKS'), '');
			}
