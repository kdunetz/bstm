<?php
/**
 * @version $Id: avatar.php 132 2010-07-26 11:50:33Z sterob $
 * @package Joo!CM
		...
		<form action="index.php?option=com_joocm" method="post" name="adminForm">
			...
			<thead><tr>
				...
				<th nowrap="nowrap" width="78%">
					<?php echo JHTML::_('grid.sort', 'CM_AVATARFILE', 'a.avatar_file', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_avatar_view'); ?>
				</th>
				<th nowrap="nowrap" width="10%">
					<?php echo JText::_('CM_AVATARUSER'); ?>
				</th>
				<th nowrap="nowrap" width="10%">
					<?php echo JText::_('CM_AVATARPREVIEW'); ?>
				</th>								
				<th nowrap="nowrap" width="1%">
					<?php echo JHTML::_('grid.sort', 'CM_PUBLISHED', 'a.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_avatar_view'); ?>
				</th>																													
				<th nowrap="nowrap" width="1%">
					<?php echo JHTML::_('grid.sort', 'CM_ID', 'a.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_avatar_view'); ?>
				</th>			
			</tr></thead>
			<tfoot>
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				if ($row->id_user) {
					...
					$row->name = JText::_('CM_STANDARDAVATAR');
				}
	...
	function editAvatar(&$row, &$lists) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_NAME'), JText::_('CM_AVATAR')); ?>");
			} else {
		...
		<form action="index.php" method="post" name="adminForm">
			<div class="col100">
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_AVATARDETAILS'); ?></legend>
					<table class="admintable" cellspacing="1"><tr>
						<td class="key">
							<label for="avatar_file" class="hasTip" title="<?php echo JText::_('CM_AVATARFILE') .'::'. JText::_('CM_AVATARFILEDESC'); ?>">
								<?php echo JText::_('CM_AVATARFILE'); ?>
							</label>
						</td><td>
