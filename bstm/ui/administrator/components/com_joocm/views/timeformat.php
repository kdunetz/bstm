<?php
/**
 * @version $Id: timeformat.php 132 2010-07-26 11:50:33Z sterob $
 * @package Joo!CM
	...
	function showTimeFormats(&$rows, $pageNav, &$lists) {
		...
		<form action="index.php?option=com_joocm" method="post" name="adminForm">
			...
			<thead>
				<tr>
					...
					<th nowrap="nowrap" width="45%">
						<?php echo JHTML::_('grid.sort', 'CM_NAME', 'f.name', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_timeformat_view'); ?>
					</th>
					<th nowrap="nowrap" width="30%">
						<?php echo JHTML::_('grid.sort', 'CM_TIMEFORMAT', 'f.timeformat', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_timeformat_view'); ?>
					</th>
					<th nowrap="nowrap" width="20%">
						<?php echo JText::_('CM_TIMEFORMATEXAMPLE'); ?>
					</th>					
					<th width="5%">
						<?php echo JText::_('CM_DEFAULT'); ?>
					</th>																			
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'CM_PUBLISHED', 'f.published', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_timeformat_view'); ?>
					</th>																																							
					<th nowrap="nowrap" width="1%">
						<?php echo JHTML::_('grid.sort', 'CM_ID', 'f.id', @$lists['filter_order_Dir'], @$lists['filter_order'], 'joocm_timeformat_view'); ?>
					</th>			
				</tr>
			</thead>
			<tfoot>
			...
			for ($i=0, $n=count($rows); $i < $n; $i++) {
				...
				$alt_default = $defaultTimeFormat ? JText::_('CM_DEFAULT') :  JText::_('CM_NOTDEFAULT');				
	...
	function editTimeFormat(&$row, &$lists) {
		...
		function submitbutton(pressbutton) {
			...
			if (trim(form.name.value) == "") {
				alert("<?php echo JText::sprintf('CM_MSGFIELDREQUIRED', JText::_('CM_NAME'), JText::_('CM_TIMEFORMAT')); ?>");
			} else {
		...
		<form action="index.php" method="post" name="adminForm">
			<div class="col width-50">
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_TIMEFORMATDETAILS'); ?></legend>		
			...
			<div class="col width-50">
				<fieldset class="adminform">
					<legend><?php echo JText::_('CM_TIMEFORMATEXAMPLE'); ?></legend>
					<table class="admintable" cellspacing="1"><tr>
						<td><?php echo JoocmHelper::formatDate(time(), $row->timeformat); ?></td>
					</tr></table>
				</fieldset>
