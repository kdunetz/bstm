<?php
/**
 * @version $Id: install.php 143 2010-08-24 14:59:50Z sterob $
 * @package Joo!BB
...
class ControllerInstall extends JController
{
	...
	function performInstall() {
		...
		if ($joocmInstaller->executeSQLFile(JOOBB_ADMINBASEPATH.DS.'uninstall.joobb.sql')) {
			...
			if ($joocmInstaller->executeSQLFile(JOOBB_ADMINBASEPATH.DS.'install.joobb.sql')) {
				$app->enqueueMessage(JText::_('BB_MSGINSTALLSUCCESS'));
			} else {
				$app->enqueueMessage($db->getErrorMsg(), 'error');
				$app->enqueueMessage(JText::_('BB_MSGINSTALLFAILED'), 'error');
			}
		} else {
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			$app->enqueueMessage(JText::_('BB_MSGINSTALLFAILED'), 'error');
	...
	function performUpdate() {
		...
		if (!$joocmInstaller->executeSQLFile(JOOBB_ADMINBASEPATH.DS.'updates'.DS.'update.sql')) {
		
			// get the db error...
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			
			// ... and redirect with component update error message...
			$app->redirect('index.php?option=com_joocm&task=joobb_install_view', JText::_('BB_MSGUPDATEFAILED'), 'error');
		}
...
//$app->enqueueMessage($query, 'error');		
		...
		if ($status == 0) {
			$app->enqueueMessage($db->getErrorMsg(), 'error');
			$app->enqueueMessage(JText::_('BB_MSGUPDATEFAILED'), 'error');
		} else {
			$app->enqueueMessage(JText::_('BB_MSGUPDATESUCCESS'));
		}
