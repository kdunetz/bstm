<?php
/**
 * @version $Id: template.php 140 2010-08-22 11:44:52Z sterob $
 * @package Joo!BB
...
class ControllerTemplate extends JController
{
	...
	function editTemplate() {
		...
		if(!$data = JoobbHelper::parseXMLFile(JOOBB_TEMPLATES.DS.basename($cid[0], ".xml"), $cid[0], 'template')) {
			$this->setRedirect('index.php?option=com_joobb&task=joobb_template_view', sprintf(JText::_('BB_MSGFILENOTFOUND'), $cid[0]), 'error'); return;
		} else {
			$row = $data;
		}
	...
	function saveTemplate() {
		...
		if(!$template = JoobbHelper::parseXMLFile(JOOBB_TEMPLATES.DS.basename($fileName, ".xml"), $fileName, 'template')) {
			$this->setRedirect('index.php?option=com_joobb&task=joobb_template_view', sprintf(JText::_('BB_MSGFILENOTFOUND'), $fileName), 'error'); return;
		}
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_TEMPLATE'), $template->name);
	...
	function defaultTemplate() {
		...
		if (count($cid)) {
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_TEMPLATE'), JText::_('BB_DEFAULT')); $msgType = 'notice';
		}
