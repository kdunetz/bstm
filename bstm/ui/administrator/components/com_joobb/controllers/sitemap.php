<?php
/**
 * @version $Id: sitemap.php 134 2010-08-13 08:03:09Z sterob $
 * @package Joo!BB
...
class ControllerSitemap extends JController
{
	...
	function showSitemap() {
		...
		$priority = array();
		$priority[] = JHTML::_('select.option', 'none', JText::_('BB_NONE'));
		$priority[] = JHTML::_('select.option', 'autodetect', JText::_('BB_AUTODETECT'));
		$lists['priority'] = JHTML::_('select.genericlist',  $priority, 'priority', 'class="inputbox" size="1"', 'value', 'text', 'autodetect');

		$changeFreq = array();
		$changeFreq[] = JHTML::_('select.option', 'none', JText::_('BB_NONE'));
		$changeFreq[] = JHTML::_('select.option', 'autodetect', JText::_('BB_AUTODETECT'));
		$changeFreq[] = JHTML::_('select.option', 'always', JText::_('BB_ALWAYS'));
		$changeFreq[] = JHTML::_('select.option', 'hourly', JText::_('BB_HOURLY'));
		$changeFreq[] = JHTML::_('select.option', 'daily', JText::_('BB_DAILY'));
		$changeFreq[] = JHTML::_('select.option', 'weekly', JText::_('BB_WEEKLY'));
		$changeFreq[] = JHTML::_('select.option', 'monthly', JText::_('BB_MONTHLY'));
		$changeFreq[] = JHTML::_('select.option', 'yearly', JText::_('BB_YEARLY'));
		$changeFreq[] = JHTML::_('select.option', 'never', JText::_('BB_NEVER'));
	...
	function performSitemap() {
		...
		if ($joobbSitemap->createSitemap($fileName, $priority, $changeFreq, $limit)) {
			$msg = JText::_('BB_MSGSITEMAPCREATESUCCESS');
		} else {
			$msg = JText::_('BB_MSGSITEMAPCREATEFAILED'); $msgType = 'error';
		}
