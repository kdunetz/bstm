<?php
/**
 * @version $Id: rank.php 134 2010-08-13 08:03:09Z sterob $
 * @package Joo!BB
...
class ControllerRank extends JController
{
	...
	function editRank() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joobb&task=joobb_rank_view', JText::sprintf('BB_MSGBEINGEDITTED', JText::_('BB_RANK'), $row->name, $editingUser->name)); return;
		}
	...
	function saveRank() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_RANK'), $row->name);
	...
	function deleteRank() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_RANK'), $row->name);
			} else {
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_RANKS'), '');
			}
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_RANK'), JText::_('BB_DELETE')); $msgType = 'notice';
		}
