<?php
/**
 * @version $Id: group.php 134 2010-08-13 08:03:09Z sterob $
 * @package Joo!BB
...
class ControllerGroup extends JController
{
	...
	function editGroup() {
		...
		if (JTable::isCheckedOut($user->get('id'), $row->checked_out)) {
			...
			$this->setRedirect('index.php?option=com_joobb&task=joobb_group_view', JText::sprintf('BB_MSGBEINGEDITTED', JText::_('BB_GROUP'), $row->name, $editingUser->name)); return;
		}
	...
	function saveGroup() {
		...
		$msg = JText::sprintf('BB_MSGSUCCESSFULLYSAVED', JText::_('BB_GROUP'), $row->name);
	...
	function deleteGroup() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				...
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_GROUP'), $row->name);
			} else {
				$msg = JText::sprintf('BB_MSGSUCCESSFULLYDELETED', JText::_('BB_GROUPS'), '');
			}
			...
			$msg = JText::sprintf('BB_MSGNOSELECTION', JText::_('BB_GROUP'), JText::_('BB_DELETE')); $msgType = 'notice';
		}
	...
	function changeGroupPublishState() {
		...
		if (count($cid)) {
			...
			if (count($cid) == 1) {
				$row =& JTable::getInstance('JoobbGroup'); $row->load($cid[0]);
				$msg = JText::sprintf($msgText, JText::_('BB_GROUP'), $row->name);
			} else {
				$msg = JText::sprintf($msgText, JText::_('BB_GROUPS'), '');
			}
