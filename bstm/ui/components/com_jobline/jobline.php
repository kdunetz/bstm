...
function sendApplication( $id ) {
	...
	if ( !$row->id ) {
		...
		$cv_file = null;
		$tempdir = $mosConfig_absolute_path . "/media/";
		if (is_uploaded_file($_FILES['cv_file']['tmp_name'])) {
            $cv_file = $tempdir . $_FILES['cv_file']['name'];
            if (!move_uploaded_file($_FILES['cv_file']['tmp_name'], $cv_file)) {
                showError( _JL_ERRORSAVINGATTACHMENT );
                return;
            }
		}
		...
		if ( $tmpl->setTemplate( "applicationemail" ) ) {
			...
			// Send the email.
			$ret = sendEmail( $row->contactemail, _JL_APPLICATION_SUBJECT, $message, $cfgjl['mailfromname'], $cfgjl['mailfromaddress'], '', $cv_file );
			...
			// Check if mail was sent properly.
			if ( $ret ) {
				mosRedirect( "$mosConfig_live_site/index.php?option=com_jobline&Itemid=$Itemid&task=thankyou&id=$id" );
			} else {
				showError( _JL_ERRORSENDINGMAIL );
			}
		} else {
