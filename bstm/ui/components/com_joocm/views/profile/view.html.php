...
class JoocmViewProfile extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joocmConfig->getConfigSettings('allow_show_users') && !$this->joocmUser->get('id')) {
			$this->app->redirect(JoocmHelper::getLink('login'), JText::_('CM_MSGNOPERMISSIONVIEWMEMBERSPROFILE'));
		}	
		
		// initialize variables
		$joocmUserView	=& JoocmUser::getInstance(JRequest::getVar('id', 0));
		...
		// handle metadata
		$this->document->setDescription(JText::sprintf('CM_PROFILEOFDESC', $joocmUserView->get('name'), $this->joocmConfig->getConfigSettings('community_name')));
		...
		if ($joocmUserView->get('show_email')) {
			...
			$joocmUserView->set('email', JText::_('CM_EMAILISNOTAVAILABLE'));
		}
