...
class JoocmViewUserList extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joocmConfig->getConfigSettings('allow_show_users') && !$this->joocmUser->get('id')) {
			$this->app->redirect(JoocmHelper::getLink('login'), JText::_('CM_MSGNOPERMISSIONVIEWMEMBERS'), 'notice');
		}
		...
		// handle page title
		$this->document->setTitle($this->joocmConfig->getConfigSettings('community_name') .' - '. JText::_('CM_MEMBERS'));
		
		// handle metadata
		$this->document->setDescription(JText::sprintf('CM_MEMBERSAT', $this->joocmConfig->getConfigSettings('community_name')));
		$this->document->setMetadata('keywords', JText::_('CM_MEMBERS'). ', '. $this->joocmConfig->getConfigSettings('keywords'));
		
		// handle bread crumb
		$this->breadCrumbs->addItem(JText::_('CM_MEMBERS'));
