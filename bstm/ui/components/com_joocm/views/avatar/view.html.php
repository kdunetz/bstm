...
class JoocmViewAvatar extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joocmUser->get('id') < 1) {
			$this->app->redirect(JoocmHelper::getLink('login'), JText::_('CM_MSGNOPERMISSIONMANAGEAVATARS'), 'notice');
		}

		if (!$this->joocmConfig->getConfigSettings('enable_avatars')) {
			$this->app->redirect(JoocmHelper::getLink('main'), JText::_('CM_MSGAVATARSNOTACTIVE'), 'notice');
		} else if ($this->joocmConfig->getConfigSettings('avatar_source') != 1) {
			$this->app->redirect(JoocmHelper::getLink('main'), JText::_('CM_MSGJOOCMAVATARSNOTACTIVE'), 'notice');
		}

		// handle page title
		$this->document->setTitle($this->joocmConfig->getConfigSettings('community_name') .' - '. JText::_('CM_MYAVATAR'));
		
		// handle bread crumb
		$this->breadCrumbs->addItem(JText::_('CM_MYAVATAR'));
