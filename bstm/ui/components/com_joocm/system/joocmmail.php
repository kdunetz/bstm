<?php
/**
 * @version $Id: joocmmail.php 133 2010-07-27 11:45:15Z sterob $
 * @package Joo!CM
...
class JoocmMail
{
	...
	function sendRegistrationMail(&$joocmUser) {
		...
		$this->subject = sprintf(JText::_('CM_ACCOUNTACTIVATIONSUBJECTUSER'), $this->siteName);
		
		switch($joocmConfig->getConfigSettings('account_activation')) {
			case 0:		// no activation needed
				$this->message = sprintf(JText::_('CM_ACCOUNTACTIVATIONNO'), $name, $this->siteName, $this->siteURL, $userName, $password);
				$this->_sendMail();
				break;
			case 1:		// activation by user
				...
				$this->message = sprintf(JText::_('CM_ACCOUNTACTIVATIONUSER'), $name, $this->siteName, $activationLink, $this->siteURL, $userName, $password);
				$this->_sendMail();
				break;
			case 2:		// activation by admin
				...
				// handle user email
				$this->message = sprintf(JText::_('CM_ACCOUNTACTIVATIONADMINUSER'), $name, $this->siteName, $this->siteURL, $userName, $password);
				$this->_sendMail();
				...
				$activationLink = $this->siteURL."index.php?option=com_joocm&task=joocmactivateprofile&activation=".$joocmUser->get('activation');
				$this->subject = sprintf(JText::_('CM_ACCOUNTACTIVATIONSUBJECTADMIN'), $this->siteName, $this->userName);

				foreach ($rows as $row) {
					$this->message = sprintf(JText::_('CM_ACCOUNTACTIVATIONADMINADMIN'), $row->name, $userName, $this->siteURL, $activationLink);
	...
	function sendConfirmationMail(&$joocmUser) {
		...
		$this->mailTo = $joocmUser->get('email');
		$this->subject = sprintf(JText::_('CM_CONFIRMATIONSUBJECT'), $this->siteName);
		$this->message = sprintf(JText::_('CM_CONFIRMATIONMESSAGE'), $joocmUser->get('name'), $this->siteURL, $this->siteName);
	...
	function sendRequestLoginMail(&$joocmUser) {
		...
		$this->mailTo = $joocmUser->get('email');
		$this->subject = sprintf(JText::_('CM_LOGINREQUESTSUBJECT'), $this->siteName);
		...
		$this->message = sprintf(JText::_('CM_LOGINREQUESTMESSAGE'), $joocmUser->get('name'), $resetLoginLink, $this->siteName);
