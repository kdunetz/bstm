<?php
/**
 * @version $Id: joocm.php 139 2010-08-20 14:41:57Z sterob $
 * @package Joo!CM
...
class JoocmController extends JController
{
	...
	function display() {
		...
		if ($joocmUser->id > 0 && !$joocmUser->get('agreed_terms') && $joocmConfig->getConfigSettings('enable_terms') && $viewName != 'terms') {
			...
			$app->enqueueMessage(JText::_('CM_MSGTERMSOFAGREEMENTNEEDED'));
		}
		...
		if ($joocmConfig->getConfigSettings('force_edit_profile') && $joocmUser->isRequiredFieldEmpty() && $joocmUser->get('agreed_terms')) {
			...
			$app->enqueueMessage(JText::_('CM_MSGREQUIREDFIELDNEEDED'));
		}
	...
	function joocmLogin() {
		...
		if ($joocmConfig->getCaptchaSettings('captcha_login')) {
			...
			if (md5($post['captcha_code']) != $session->get('captcha_code'))  {
				$this->setRedirect(JoocmHelper::getLink('login'), JText::_('CM_CAPTCHACODEDONOTMATCH')); return;
			}
		}

		if (empty($username)) {
			$this->setRedirect(JoocmHelper::getLink('login'), JText::_('CM_MSGENTERUSERNAME')); return;
		} else if (empty($password)) {
			$this->setRedirect(JoocmHelper::getLink('login'), JText::_('CM_MSGENTERPASS')); return;
		}
	...
	function joocmLogout() {
		...
		if(!JError::isError($error)) {
			$app->enqueueMessage(JText::_('CM_MSGLOGGEDOUT'));
		}
	...
	function joocmSaveAccount() {
		...
		if ($joocmUser->get('id') < 1) {
			$msg = JText::_('CM_MSGNOPERMISSIONEDITACCOUNT'); $msgType = 'notice';
		} else {
			$msg = JText::_('CM_MSGACCOUNTSAVED');
	...
	function joocmSaveProfile() {
		...
		if ($joocmUser->get('id') < 1) {
			$msg = JText::_('CM_MSGNOPERMISSIONEDITPROFILE'); $msgType = 'notice';
		} else {		
			$msg = JText::_('CM_MSGPROFILESAVED');
	...
	function joocmSaveSettings() {
		...
		if ($joocmUser->get('id') < 1) {
			$msg = JText::_('CM_MSGNOPERMISSIONEDITSETTINGS'); $msgType = 'notice';
		} else {
			$msg = JText::_('CM_MSGSETTINGSSAVED');
	...
	function joocmAgreedTerms() {
		...
		if ($joocmUser->get('id') < 1) {
			$msg = JText::_('CM_MSGNOPERMISSIONAGREETERMS'); $msgType = 'notice';
		} else {
			$msg = JText::_('CM_MSGAGREEDTERMS');
	...
	function joocmRegister() {
		...
		if ($joocmConfig->getCaptchaSettings('captcha_register')) {
			...
			if (md5($post['captcha_code']) != $session->get('captcha_code'))  {
				...
				$this->setRedirect(JRoute::_('index.php?option=com_joocm&view=register&Itemid='.$this->Itemid, false), JText::_('CM_CAPTCHACODEDONOTMATCH'), 'notice'); return;
			}
		}
		...
		if ($userId) {
			...
			if ($db->query()) {
				...
				if ($joocmMail->sendRequestLoginMail($joocmUser)) {
					...
					$msg = JText::_('CM_MSGMAILNOTSENT'); $msgType = 'notice';
				}
			} else {
				$link = JRoute::_('index.php?option=com_joocm&view=requestlogin&Itemid='.$Itemid, false);
			...
			$link = JRoute::_('index.php?option=com_joocm&view=requestlogin&Itemid='.$Itemid, false);
			$msg = JText::_('CM_MSGUSERNOTFOUND'); $msgType = 'notice';
		}
	...
	function joocmSaveAvatar() {
		...
		if ($joocmUser->get('id') < 1) {
			$msg = JText::_('CM_MSGNOPERMISSIONMANAGEAVATARS'); $msgType = 'notice';
		} else {
			$msg = JText::_('CM_MSGAVATARSAVESUCCESS');
	...
	function joocmUploadAvatar() {
		...
		if ($joocmAvatar->uploadAvatar(JRequest::getVar('avatarfile', '', 'files', 'array'), JRequest::getVar('avatarurl', ''), JRequest::getVar('my_avatar_id', 0, '', 'int'))) {
			$msg = JText::_('CM_MSGAVATARUPLOADSUCCESS');
		}
	...
	function joocmDeleteAvatar() {
		...
		if ($joocmAvatar->deleteAvatar(JRequest::getVar('id_avatar', 0, '', 'int'))) {
			$msg = JText::_('CM_MSGAVATARDELETESUCCESS');
		}
