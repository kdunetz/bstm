...
<script language="javascript" type="text/javascript">
	...
	function addAttachmentField() {
		...
		removeLink.appendChild(document.createTextNode('<?php echo JText::_('BB_DELETEATTACHMENT'); ?>'));
...
<form action="index.php" method="post" id="josForm" name="josForm" class="form-validate" enctype="multipart/form-data">
	...
	<div class="jbBoxOuter"><div class="jbBoxInner">
		<div class="jbLeft jbMargin5">
			<label class="jbLabel"><?php echo JText::_('BB_TOPICICON'); ?></label>
			...
			if ($this->enableGuestName) : 
				...
				<label for="guest_name" class="jbLabel"><?php echo JText::_('BB_GUESTNAME'); ?></label>
			...
			<label for="subject" class="jbLabel"><?php echo JText::_('BB_SUBJECT'); ?></label>
			<input class="jbInputBox jbField required" type="text" id="subject" name="subject" size="50" maxlength="255" value="<?php echo $this->post->subject; ?>" />
			<br clear="all" />
            <label for="text" class="jbLabel"><?php echo JText::_('BB_MESSAGE'); ?></label>
			<div class="jbField"><?php echo $this->editor->getButtons('text'); ?></div>
			<br clear="all" />
			<div class="jbLeft">
				<?php echo $this->editor->display('text', 'jbEditor required', $this->post->text, '100%', '275', '100%', '15');
                if ($this->joocmCaptcha->enabled) : ?>
                <fieldset>
                    <legend class="jbLegend"><?php echo JText::_('BB_HUMANVERIFICATION'); ?></legend>
                    <p><img src="<?php echo $this->joocmCaptcha->getImageSource(); ?>" title="<?php echo JText::_('BB_CAPTCHACODE'); ?>" alt="<?php echo JText::_('BB_CAPTCHACODE'); ?>" /></p>
				...
				<fieldset>
					<legend class="jbLegend"><?php echo JText::_('BB_POSTACTION'); ?></legend>
					...
					<div class="jbLeft" style="width: 33%;">
						<button type="submit" class="<?php echo $this->buttonPreview->class; ?>" title="<?php echo $this->buttonPreview->title; ?>" onclick="submitbutton('joobbpreviewtopic');"><span><?php echo $this->buttonPreview->text; ?></span></button>
					</div>
					<div class="jbLeft" style="width: 33%;">
				...
				<fieldset>
					<legend class="jbLegend"><?php echo JText::_('BB_POSTOPTIONS'); ?></legend>
					<label for="type" class="jbLabel"><?php echo JText::_('BB_POSTTOPICAS'); ?></label>
					<?php echo $this->lists['type']; ?><br clear="all" />
					...
					<label for="subscribe" class="jbLabel"><?php echo JText::_('BB_SUBSCRIBE'); ?></label>
					<?php echo $this->lists['subscribe']; ?>
				</fieldset>
				<?php if (count($this->attachments)) : ?>
				<fieldset>
					<legend class="jbLegend"><?php echo JText::_('BB_ATTACHMENTS'); ?></legend>
					<div class="jbAttachmentBox">
						...
						foreach ($this->attachments as $attachment) { ?>
							<li>
								...
								<input type="checkbox" name="attachments[]" value="<?php echo $attachment->id; ?>" onClick="" ><?php echo JText::_('BB_CHECKTODELETEATTACHMENT'); ?></input>
							</li><?php	
						} ?>
						</ul>
					</div>
				...
				<fieldset>
					<legend class="jbLegend"><?php echo JText::_('BB_UPLOADATTACHMENT'); ?></legend>
					<p><?php echo JText::sprintf('BB_MAXALLOWEDATTACHMENTS', $this->joobbAttachment->attachmentMax); ?></p>
					<p><?php echo JText::sprintf('BB_MAXATTACHMENTSIZE', $this->joobbAttachment->attachmentMaxFileSize); ?></p>
					<p><?php echo JText::sprintf('BB_ALLOWEDATTACHMENTS', $this->joobbAttachment->attachmentFileTypes); ?></p>
					<div id="attachmentUpload">
						<input type="file" name="attachmentFiles[]" size="40" class="jbInputBox" />
						<p><a style="cursor:pointer;" onclick="addAttachmentField(); return false;"><?php echo JText::_('BB_ADDOTHERATTACHMENT'); ?></a></p>
					</div>
				</fieldset>
				<?php endif; ?>		
			</div>
	...
	<input type="hidden" name="task" value="joobbsavetopic" />
