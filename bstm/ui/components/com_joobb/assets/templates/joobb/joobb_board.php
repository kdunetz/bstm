...
for ($i=0; $i < count($this->categories); $i ++) :
	...
	$javaScript .= '$j("#idMinMax_'.$this->category->id.'").attr("title", "'.JText::_('BB_MINIMIZE').'");';
	$javaScript .= '} else {';
	$javaScript .= '$j("#jbForumBlock_'.$this->category->id.'").hide("slow"); $j("#idMinMax_'.$this->category->id.'").addClass("jbImageMax");';
	$javaScript .= '$j("#idMinMax_'.$this->category->id.'").attr("title", "'.JText::_('BB_MAXIMIZE').'");';
	$javaScript .= '}});';
endfor;
$javaScript .= '}catch(e){}});';
$this->document->addScriptDeclaration($javaScript); ?>
<div class="jbMargin5 jbMarginBottom10">
	<div class="jbLeft jbFont13 jbPaddingTop5"><?php echo $this->currentTime . JText::_(' - '). JText::_('BB_ALLTIMESARE') .' '. JoocmHelper::getCurrentTimeZoneName(true); ?></div>
