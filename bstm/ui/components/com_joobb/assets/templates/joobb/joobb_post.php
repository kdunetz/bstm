...
<div class="jbBoxTopLeft"><div class="jbBoxTopRight"><div class="jbBoxTop">
	<div class="jbTextHeader">
		<span class="jbLeft jbMarginRight5"><?php echo JText::_('BB_BY'); ?></span><?php
		...
		<span class="jbLeft jbMarginLeft5"><?php echo JText::_('BB_ON'); ?></span>
...
<div class="jbPadding10 jbBorderTop jbBorderBottom" id="<?php echo $this->post->pid ?>">
	<div class="jbLeft jbWidth80">
		...
		<div class="jbAttachmentBox">
			<strong><?php echo JText::_('BB_ATTACHMENTS'); ?></strong><br />
	...
	if ($this->post->registerDate) {
		$this->post->avatarFileAlt = $this->post->avatarFileAlt . ' | ' . JText::_('BB_JOINEDBOARD').': ' . $this->post->registerDate;
	}
	if ($this->post->lastvisitDate) {
		$this->post->avatarFileAlt = $this->post->avatarFileAlt . ' | ' . JText::_('BB_LASTVISIT').': ' .$this->post->lastvisitDate;
	}
	if ($this->post->authorLink) : ?>
	...
	if ($this->post->posts) {
		echo JText::_('BB_POSTS') .': '. $this->post->posts .'<br />';
	}
	if ($this->post->authorLink) : ?>
	...
	<span class="jbVerticalAlignTop"><?php
		if ($this->post->onlineState) : 
			echo JText::_('BB_MEMBERISONLINE');
		else :
			echo JText::_('BB_MEMBERISOFFLINE');
		endif; ?></span><br /><?php
	endif;
	if ($this->post->postsByAuthorLink) : ?>
		<a href="<?php echo $this->post->postsByAuthorLink; ?>"><?php echo JText::_('BB_VIEWALLUSERSPOSTS'); ?></a><br /><?php
	endif; ?>
    </div>
	</div>
	<br clear="all" />
	<div class="jbSignature jbMarginBottom10 jbBorderBottomDeshed">
		...
		$editor = ($this->post->editor != '') ? JText::_('BB_BY').' '.$this->post->editor : ''; 
		echo JText::_('BB_LASTEDIT').' '.JText::_('BB_ON').' '.$this->post->lastEditDate.' '.$editor; ?>
	...
	if ($this->post->showSubscription) : 
		...
		<a href="<?php echo $this->post->subscriptionLink; ?>">
			<img src="<?php echo $this->joobbTemplate->themePathLive.DL.'images'.DL.'subscriptonAdd.png'; ?>" alt="<?php echo JText::_('BB_SUBSCRIBE'); ?>" />
		</a><?php
		else : ?>
		<a href="<?php echo $this->post->subscriptionLink; ?>">
			<img src="<?php echo $this->joobbTemplate->themePathLive.DL.'images'.DL.'subscriptonDelete.png'; ?>" alt="<?php echo JText::_('BB_UNSUBSCRIBE'); ?>" />
		</a><?php
		endif;
	else : ?>
		<a href="javascript:scroll(0,0)">
			<img src="<?php echo $this->joobbTemplate->themePathLive.DL.'images'.DL.'top.png'; ?>" alt="<?php echo JText::_('BB_TOP'); ?>" title="<?php echo JText::_('BB_TOP'); ?>" />
		</a><?php
	endif; ?>
	</div>
	<div align="right"><?php
	if ($this->post->buttonReportPost) : ?>
		<a href="<?php echo $this->post->buttonReportPost->href; ?>" class="<?php echo $this->post->buttonReportPost->class; ?> jbRight jbMarginLeft5" title="<?php echo $this->post->buttonReportPost->title; ?>"><span><?php echo $this->post->buttonReportPost->text; ?></span></a><?php
	endif;
	if ($this->post->buttonDelete) : 
		if ($this->post->buttonDelete->captcha) :
		JHTML::_('behavior.modal', 'a.modal');	?>
        <a href="<?php echo $this->post->buttonDelete->href.'&tmpl=component'; ?>" class="modal <?php echo $this->post->buttonDelete->class; ?> jbRight jbMarginLeft5" title="<?php echo $this->post->buttonDelete->title; ?>" rel="{handler: 'iframe', size: {x: 250, y: 250}, overlayOpacity: 0.3}"><span><?php echo $this->post->buttonDelete->text; ?></span></a><?php 
		else : ?>
		<a href="<?php echo $this->post->buttonDelete->href; ?>" class="<?php echo $this->post->buttonDelete->class; ?> jbRight jbMarginLeft5" title="<?php echo $this->post->buttonDelete->title; ?>"><span><?php echo $this->post->buttonDelete->text; ?></span></a><?php
		endif;
	endif;
	if ($this->post->buttonEdit) : ?>
		<a href="<?php echo $this->post->buttonEdit->href; ?>" class="<?php echo $this->post->buttonEdit->class; ?> jbRight jbMarginLeft5" title="<?php echo $this->post->buttonEdit->title; ?>"><span><?php echo $this->post->buttonEdit->text; ?></span></a><?php
	endif;
	if ($this->post->buttonQuote) : ?>
		<a href="<?php echo $this->post->buttonQuote->href; ?>" class="<?php echo $this->post->buttonQuote->class; ?> jbRight" title="<?php echo $this->post->buttonQuote->title; ?>"><span><?php echo $this->post->buttonQuote->text; ?></span></a><?php
	endif; ?>
	</div><br clear="all" />
</div>
</div></div>
