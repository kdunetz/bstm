<?php
/**
 * @version $Id: view.html.php 194 2010-10-25 21:33:22Z sterob $
 * @package Joo!BB
...
class JoobbViewForum extends JView
{
	...
	function display($tpl = null) {
		...
		if (!$forumId) {
			$this->messageQueue->addMessage(JText::_('BB_MSGREQUESTNOTPERFORMED'));
		...
		if (!$joobbAuth->getAuth('auth_read', $forum->id)) {
			$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGREADFORUM')));
			$this->app->redirect(JRoute::_('index.php?option=com_joobb&view=board&Itemid='.$this->Itemid, false));
		}
		...
		$this->assignRef('searchInputBoxText', JText::_('BB_SEARCHTHISFORUM'));
