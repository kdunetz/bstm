...
class JoobbViewMoveTopic extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joobbUser->getRole($topic->id_forum) < 2) {
			$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGMOVETOPIC')));
		...
		// handle page title
		$this->document->setTitle(JText::_('BB_MOVETOPIC'));
		
		// handle bread crumb
		$this->breadCrumbs->addBreadCrumb(JText::_('BB_MOVETOPIC'), '');
