...
class JoobbViewSubscriptions extends JView
{
	...
	function display($tpl = null) {
		...
		if ($this->joobbUser->get('id') < 1) {
			$this->messageQueue->addMessage(JText::_('BB_MSGNOPERMISSIONVIEWSUBSCRIPTIONS'));
		...
		// handle page title
		$this->document->setTitle(JText::_('BB_MYSUBSCRIPTIONS'));
		
		// handle bread crumb
		$this->breadCrumbs->addBreadCrumb(JText::_('BB_MYSUBSCRIPTIONS'));
	...
	function &getSubscription($index = 0) {
		...
		$subscription->subscriptionLink = JRoute::_('index.php?option=com_joobb&task=joobbunsubscribetopic&topic='.$subscription->id.'&Itemid='. $this->Itemid, false);
		...
		if ($subscription->author) {
			...
			if ($subscription->guest_author) {
				...
				$subscription->author = JText::_('BB_GUEST');
			}
		}
		...
		if ($subscription->poster) {
			...
			if ($subscription->guest_poster) {
				...
				$subscription->poster = JText::_('BB_GUEST');
			}
		}
