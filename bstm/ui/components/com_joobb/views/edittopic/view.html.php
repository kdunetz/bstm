<?php
/**
 * @version $Id: view.html.php 196 2010-11-01 09:20:31Z sterob $
 * @package Joo!BB
...
class JoobbViewEditTopic extends JView
{
	...
	function display($tpl = null) {
		...
		if ($forumId == 0) {
			...
			if ($forumId == 0) {
				$this->messageQueue->addMessage(JText::_('BB_MSGFORUMNOTSPECIFIED'));
			...
			if ($this->topic->id == 0 ||																	// ... we are about to create a new topic
				...
				if ($this->forum->locked) {
					...
					if (!$joobbAuth->getAuth($authTopicAll, $this->forum->id)) {
						...
						$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGFORUMLOCKED'), $this->forum->name));
		...
		if (!$canPost) {
			if ($this->topic->id) {
				...
				$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGEDITTOPIC')));
			} else {
				$link = JRoute::_('index.php?option=com_joobb&view=forum&forum='.$this->forum->id.'&Itemid='.$this->Itemid, false);
				$this->messageQueue->addMessage(sprintf(JText::_('BB_MSGNOPERMISSION'), JText::_('BB_MSGPOSTTOPIC')));
			}
		...
		$lists['enable_bbcode'] = JHTML::_('select.booleanlist', 'enable_bbcode', '', $enableBBCode, JText::_('BB_YES'), JText::_('BB_NO'));
		$lists['enable_emotions'] = JHTML::_('select.booleanlist', 'enable_emotions', '', $enableEmotions, JText::_('BB_YES'), JText::_('BB_NO'));
		$lists['subscribe'] = JHTML::_('select.booleanlist', 'subscribe', '', $subscribe, JText::_('BB_YES'), JText::_('BB_NO'));

		$topictype = array();
		$topictype[] = JHTML::_('select.option', 0, JText::_('BB_NORMAL'), 'value', 'text');
		if ($joobbAuth->getAuth('auth_sticky', $this->forum->id)) {
			$topictype[] = JHTML::_('select.option', 1, JText::_('BB_STICKY'), 'value', 'text');
		}
		if ($joobbAuth->getAuth('auth_announce', $this->forum->id)) {
			$topictype[] = JHTML::_('select.option', 2, JText::_('BB_ANNOUNCE'), 'value', 'text');
		}	
