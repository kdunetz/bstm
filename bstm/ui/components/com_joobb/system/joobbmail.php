<?php
/**
 * @version $Id: joobbmail.php 196 2010-11-01 09:20:31Z sterob $
 * @package Joo!BB
...
class JoobbMail extends JoocmMail
{
	...
	function sendNotifyOnReplyMail($joobbPost) {
		...
		$this->subject = sprintf(JText::_('BB_NOTIFYONREPLYSUBJECT'), $this->siteName);
		...
		foreach ($notifies as $notify) {
			...
			$this->message = sprintf(JText::_('BB_NOTIFYONREPLYMESSAGE'), $notify->name, $joobbPost->subject, $topicLink, $this->siteName);
			$this->_sendMail();
		}
	}
	...
	function sendReportPostMail($joobbPost, $reportComment) {
		...
		// subject
		$this->subject = sprintf(JText::_('BB_REPORTPOSTSUBJECT'), $this->siteName);
		...
		if (count($systemUsers)) {
			foreach ($systemUsers as $systemUser) {
				...
				$this->message = sprintf(JText::_('BB_REPORTPOSTMESSAGE'), $systemUser->name, $reportComment, $topicLink, $this->siteName);
				$this->_sendMail();
			}
		} else {
			$messageQueue->addMessage(JText::_('BB_MSGREPORTNOTSENT')); return false;
		}
