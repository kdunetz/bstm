<?php
/**
 * @version $Id: joobbbuttonset.php 83 2010-04-16 18:19:40Z sterob $
 * @package Joo!BB
...
class JoobbButtonSet
{
	...
	function loadXML() {
		...
		if (file_exists($this->buttonsPath.DS.$this->xmlFile)) {
			...
			if ($buttonSet) {
				...
				$messageQueue->addMessage(JText::sprintf('BB_MSGXMLPARSINGERROR', JText::_('BB_BUTTONSET'), $this->buttonsPath.DS.$this->xmlFile));
			}
		} else {
			$messageQueue->addMessage(JText::sprintf('BB_MSGRESSOURCENOTFOUND', JText::_('BB_BUTTONSET'), $this->buttonsPath.DS.$this->xmlFile));
		}
	}
	...
	function loadLanguage() {
		...
		if (is_file($langFile)) {
			...
			$messageQueue->addMessage(JText::sprintf('BB_MSGNOLANGUAGEFILE', JText::_('BB_BUTTONSET')));
		}
	}
