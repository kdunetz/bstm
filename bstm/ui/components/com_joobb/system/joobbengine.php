<?php
/**
 * @version $Id: joobbengine.php 180 2010-10-03 12:36:37Z sterob $
 * @package Joo!BB
...
//print_r($matches);
		switch ($tag) {
			...
			case 'code':
				...
				$replacement = '[noparse]<span class="codetext">'.JText::_('BB_CODE').'</span>'. $replacement .'[/noparse]';
				break;
			...
			case 'img':
				...
				$widthString = ''; $heightString = ''; $imageTitle = JText::_('BB_IMAGETITLEEMBEDDED');
				...
				if (is_array($scaledImageDim)) {
					...
					$imageTitle = JText::_('BB_IMAGETITLENOIMAGE');
					$innerString = JURI::root().DL.'media'.DL.'joobb'.DL.'images'.DL.'no_image.png';
				}

				$replacement = '<img src="'. $innerString .'"'.$widthString.$heightString.' title="'.$imageTitle.'" border="0" />';
				break;
			...
			case 'quote':
				$quotedUser = (isset($argument) && $argument != '') ? $argument : JText::_('BB_UNKNOWN');
				$replacement = '<span class="quotebyuser">'.JText::_('BB_QUOTEBY').' '.$quotedUser.':</span><span class="quote">' . $innerString . '</span>';
				break;
