<?php
/**
 * @version $Id: joobbiconset.php 105 2010-05-18 22:02:39Z sterob $
 * @package Joo!BB
...
class JoobbIconSet
{
	...
	function JoobbIconSet($xmlFile) {
		...
		if (file_exists(JOOBB_ICONS.DS.$this->name.DS.$xmlFile)) {
			...
			if ($iconSet) {
				...
				$messageQueue->addMessage(JText::sprintf('BB_MSGXMLPARSINGERROR', JText::_('BB_ICONSET'), JOOBB_ICONS.DS.$this->name.DS.$xmlFile));
			}
		} else {
			$messageQueue->addMessage(JText::sprintf('BB_MSGRESSOURCENOTFOUND', JText::_('BB_ICONSET'), JOOBB_ICONS.DS.$this->name.DS.$xmlFile));
		}		
	}
	...
	function loadLanguage($xmlFile) {
		...
		if (is_file($langFile)) {		
			...
			$messageQueue->addMessage(JText::sprintf('BB_MSGNOLANGUAGEFILE', JText::_('BB_ICONSET')));
		}
	}
