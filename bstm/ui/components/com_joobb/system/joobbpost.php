<?php
/**
 * @version $Id: joobbpost.php 195 2010-10-28 16:18:12Z sterob $
 * @package Joo!BB
...
class JoobbPost
{
	...
	function assignSubscription($post) {
		...
		if ($post->id_first_post == $post->id && $this->joobbUser->get('id') > 0 ) {
			...
			if (JoobbModel::getSubscription($post->id_topic, $this->joobbUser->get('id'))) {
				$topicSubscription = 0;
				$subscriptionLink = JRoute::_('index.php?option=com_joobb&task=joobbunsubscribetopic&topic='.$post->id_topic.'&Itemid='. $this->Itemid, false);
			} else {
				$topicSubscription = 1;
				$subscriptionLink = JRoute::_('index.php?option=com_joobb&task=joobbsubscribetopic&topic='.$post->id_topic.'&Itemid='. $this->Itemid, false);
			}
	...
	function assignAuthorRole($post) {		
		...
		if ($joobbUser) {
			...
			switch ($role) {
				case 0:
					$post->authorRole = JText::_('BB_GUEST');
					$post->authorClass = JText::_('jbGuest');
					break;
				case 1:
					$post->authorRole = JText::_('BB_REGISTERED');
					$post->authorClass = JText::_('jbRegistered');
					break;
				case 2:
					$post->authorRole = JText::_('BB_PRIVATE');
					$post->authorClass = JText::_('jbPrivate');
					break;
				case 3:
					$post->authorRole = JText::_('BB_MODERATOR');
					$post->authorClass = JText::_('jbModerator');
					break;
				case 4:
					$post->authorRole = JText::_('BB_ADMINISTRATOR');
	...
	function assignOnlineState($post) {
		...
		$post->onlineStateAlt = JText::_('BB_OFFLINE');
		...
		if ($post->show_online_state) {
			...
			if ($db->loadResult()) {
				...
				$post->onlineStateAlt = JText::_('BB_ONLINE');
			}
		}
	}
