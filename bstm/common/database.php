...
*/
  ...
  function tep_db_connect($server = DB_SERVER, $username = DB_SERVER_USERNAME, $password = DB_SERVER_PASSWORD, $database = DB_DATABASE, $link = 'db_link') {
    ...
    if (USE_PCONNECT == 'true') {
      $$link = mysql_pconnect($server, $username, $password) or tep_db_fatal_error($query, $errno, $error);
    } else {
      $$link = mysql_connect($server, $username, $password) or tep_db_fatal_error($query, $errno, $error);
    }
  ...
  function tep_db_fatal_error($query, $errno, $error) { 
    $errorMessage = file_get_contents("fatalDatabaseError.html");
    die($errorMessage);
    //die('<center><font color="#000000"><b>' . $errno . ' - ' . $error . '<br><br>' . $query . '<br><br><small><font color="#ff0000"></font></small><br><br></b></font>');
  }
