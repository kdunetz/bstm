<?php
  ...
  {
     $urlPrefix = "tip.php?";
     $urlPrefixShort = "tip.php";
  }
  else
  {
     $urlPrefix = "index.php?view=tip&";
     $urlPrefixShort = "index.php?view=tip";
  }
...
/* do the save actions up here so we can redirect if we want to */
  ...
  {
      tep_db_query("DELETE FROM tip WHERE tip_id = " . $_GET['tip_id']);
  ...
  {
      tep_db_query("UPDATE tip SET " . $_GET['field'] . "= '" .  $_GET['value'] . "' WHERE tip_id = " . $_GET['tip_id']);
  ...
  {
if (empty($_POST['tip_name'])) {
   $messageStack->add('Tip Name is required');
}
...
tep_db_perform('tip', $sql_data_array, 'update', "tip_id = " . $_POST['tip_id']);
...
$objects_query = tep_db_query("select * from tip WHERE tip_id != " . cs($_POST['tip_id'],"0") . " AND TIP_NAME= '" . $_POST['tip_name'] . "'");
...
tep_db_perform('tip', $sql_data_array, 'insert', "tip_id = " . $_POST['tip_id']);
...
<?php 
  ...
  {
     echo "Please confirm you would like to delete this tip<br><hr>";
     if (isset($_GET['redirect_url']))
     {
        $redirect_url = "redirect_url=" . urlencode($_GET['redirect_url']);
        $redirect_url_with_ampersand = "&" . $redirect_url;
     }
     echo "<a href=\" . $urlPrefix . \"action=delete_confirm&tip_id=" . $_GET['tip_id'] . $redirect_url_with_ampersand . ">Yes</a>";
     echo "<hr>";
     if (isset($_GET['redirect_url']))
     {
  ...
  {
    if (!isset($_GET['tip_id']) && isset($_POST['tip_id']))
        $_GET['tip_id'] = $_POST['tip_id'];

/* KAD leave off the ending quotes because it is an equation on the end */
     $objects_query = tep_db_query("SELECT * FROM tip WHERE tip.tip_id = " . $_GET['tip_id']);
     ...
     echo "<a href=" . $urlPrefix . "action=display_edit_form&tip_id=" . $objects['tip_id'] . ">Edit</a>
<table width=100% border=1>
<tr>
  <td>Tip Id:</td>
  <td>" .  $objects['tip_id']  . "</td>
</tr>
<tr>
  <td>Tip Name: *</td>
  <td>" .  $objects['tip_name']  . "</td>
</tr>
<tr>
  <td>Comments:</td>
...
<tr>
  <td>Tip Name: *</td>
  <td><input name=tip_name value=\"" . '' . "\"></td>
</tr>
<tr>
  <td>Comments:</td>
  <td>
<textarea rows=6 cols=6 name=comments>" . $objects['comments'] . "</textarea></td>
  ...
  {
     $objects_query = tep_db_query("select * from tip WHERE tip_id = " . $_GET['tip_id']);
     $objects = tep_db_fetch_array($objects_query);
    echo "<form action=\"" . $urlPrefixShort . "\" method=\"post\">
...
<tr>
  <td>Tip Name: *</td>
  <td><input name=tip_name value=\"" . $objects['tip_name'] . "\"></td>
</tr>
<tr>
  <td>Comments:</td>
  <td>
<textarea rows=6 cols=6 name=comments>" . $objects['comments'] . "</textarea></td>
  ...
  $rows = 0;
  echo "<a href=\" . $urlPrefix . \"action=display_create_form>Add Tip</a>";
  echo "<table border=1 width=\"100%\">";
  echo "<tr><td align=center><b>Action</b></td><td align=center><b>Name</b></td></tr>\n";

  $objects_query = tep_db_query("SELECT * FROM tip ORDER BY tip_name");
  while ($objects = tep_db_fetch_array($objects_query)) 
  {
     $rows++;

     echo "<tr><a name=\"" . $objects['tip_id'] . "\"></a><td><a href=" . $URLPrefix . "action=delete&tip_id=" . $objects['tip_id'] . ">Delete</a><br><a href=" . $URLPrefix . "action=display_edit_form&tip_id=" . $objects['tip_id'] . ">Edit</a>";
echo "</td>  <td><a href=tip.php?action=display&tip_id=" . $objects['tip_id'] . ">" . displayField($objects['tip_name'],'STRING','') . "</a></td></tr>\n";
