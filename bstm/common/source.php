<?php
  ...
  if ($mobilesite == 'true')
  {
     $urlPrefix = "source.php?";
     $urlPrefixShort = "source.php";
  }
  else
  {
     $urlPrefix = "index.php?view=source&";
     $urlPrefixShort = "index.php?view=source";
  }
...
/* do the save actions up here so we can redirect if we want to */
  ...
  {
      tep_db_query("DELETE FROM source WHERE source_id = " . $_GET['source_id']);
  ...
  {
      tep_db_query("UPDATE source SET " . $_GET['field'] . "= '" .  $_GET['value'] . "' WHERE source_id = " . $_GET['source_id']);
...
if (empty($_POST['source_name'])) {
   $messageStack->add('Source Name is required');
}
...
tep_db_perform('source', $sql_data_array, 'update', "source_id = " . $_POST['source_id']);
...
$objects_query = tep_db_query("select * from source WHERE source_id != " . cs($_POST['source_id'],"0") . " AND SOURCE_NAME= '" . $_POST['source_name'] . "'");
...
tep_db_perform('source', $sql_data_array, 'insert', "source_id = " . $_POST['source_id']);
...
<?php 
  ...
  {
     echo "Please confirm you would like to delete this source<br><hr>";
     if (isset($_GET['redirect_url']))
     {
        $redirect_url = "redirect_url=" . urlencode($_GET['redirect_url']);
        $redirect_url_with_ampersand = "&" . $redirect_url;
     }
     echo "<a href=\" . $urlPrefix . \"action=delete_confirm&source_id=" . $_GET['source_id'] . $redirect_url_with_ampersand . ">Yes</a>";
     echo "<hr>";
     if (isset($_GET['redirect_url']))
     {
  ...
  {
    if (!isset($_GET['source_id']) && isset($_POST['source_id']))
        $_GET['source_id'] = $_POST['source_id'];

/* KAD leave off the ending quotes because it is an equation on the end */
     $objects_query = tep_db_query("SELECT * FROM source WHERE source.source_id = " . $_GET['source_id']);
     ...
     echo "<a href=" . $urlPrefix . "action=display_edit_form&source_id=" . $objects['source_id'] . ">Edit</a>
<table width=100% border=1>
<tr>
  <td>Source Id:</td>
  <td>" .  $objects['source_id']  . "</td>
</tr>
<tr>
  <td>Source Name: *</td>
...
<tr>
  <td>Source Name: *</td>
  <td><input name=source_name value=\"" . '' . "\"></td>
</tr>
<tr>
  <td>Comments:</td>
  <td>
<textarea rows=6 cols=6 name=comments>" . $objects['comments'] . "</textarea></td>
  ...
  {
     $objects_query = tep_db_query("select * from source WHERE source_id = " . $_GET['source_id']);
     $objects = tep_db_fetch_array($objects_query);
    echo "<form action=\"" . $urlPrefixShort . "\" method=\"post\">
...
<tr>
  <td>Source Name: *</td>
  <td><input name=source_name value=\"" . $objects['source_name'] . "\"></td>
</tr>
<tr>
  <td>Comments:</td>
  <td>
<textarea rows=6 cols=6 name=comments>" . $objects['comments'] . "</textarea></td>
  ...
  $rows = 0;
  echo "<a href=\" . $urlPrefix . \"action=display_create_form>Add Source</a>";
  echo "<table border=1 width=\"100%\">";
  echo "<tr><td align=center><b>Action</b></td><td align=center><b>Name</b></td></tr>\n";

  $objects_query = tep_db_query("SELECT * FROM source ORDER BY source_name");
  while ($objects = tep_db_fetch_array($objects_query)) 
  {
     $rows++;

     echo "<tr><a name=\"" . $objects['source_id'] . "\"></a><td><a href=" . $URLPrefix . "action=delete&source_id=" . $objects['source_id'] . ">Delete</a><br><a href=" . $URLPrefix . "action=display_edit_form&source_id=" . $objects['source_id'] . ">Edit</a>";
echo "</td>  <td><a href=source.php?action=display&source_id=" . $objects['source_id'] . ">" . displayField($objects['source_name'],'STRING','') . "</a></td></tr>\n";
